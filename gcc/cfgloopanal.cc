...
/* Return true if BB profile can be used to determine the expected number of
   ...
   RELIABLE is set if profile indicates that the returned value should be a
   realistic estimate.  */
				     ...
				     bool *reliable)
{
  ...
  if (!header_count.initialized_p () || !header_count.nonzero_p ())
    return false;
...
maybe_flat_loop_profile (const class loop *loop)
{
  ...
  if (reliable)
    {
      int64_t intret = (ret + sreal (1, -1)).to_int ();
  ...
  int64_t intret = (ret * sreal (9, -3) + sreal (1, -1)).to_int ();
