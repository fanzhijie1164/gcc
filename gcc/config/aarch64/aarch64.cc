...
static const struct cpu_addrcost_table ftc66x_addrcost_table =
{
    {
      1, /* hi  */
      0, /* si  */
...
static const struct cpu_addrcost_table ftc86x_addrcost_table =
{
    {
      1, /* hi  */
      0, /* si  */
...
static const struct cpu_regmove_cost ftc66x_regmove_cost =
{
  1, /* GP2GP  */
  /* Spilling to int<->fp instead of memory is recommended so set
     realistic costs compared to memmov_cost.  */
...
static const struct cpu_regmove_cost ftc86x_regmove_cost =
{
  1, /* GP2GP  */
  /* Avoid the use of slow int<->fp moves for spilling by setting
     their cost higher than memmov_cost.  */
...
static const advsimd_vec_cost ftc66x_advsimd_vector_cost =
{
  2, /* int_stmt_cost  */
  2, /* fp_stmt_cost  */
  0, /* ld2_st2_permute_cost  */
...
static const struct cpu_vector_cost ftc66x_vector_cost =
{
  1, /* scalar_int_stmt_cost  */
  1, /* scalar_fp_stmt_cost  */
  4, /* scalar_load_cost  */
  ...
  1, /* cond_taken_branch_cost  */
  1, /* cond_not_taken_branch_cost  */
  &ftc66x_advsimd_vector_cost, /* advsimd  */
  nullptr, /* sve  */
  nullptr  /* issue_info  */
};
...
static const struct cpu_vector_cost ftc86x_vector_cost =
{
  1, /* scalar_int_stmt_cost  */
  1, /* scalar_fp_stmt_cost  */
  4, /* scalar_load_cost  */
  ...
  1, /* cond_taken_branch_cost  */
  1, /* cond_not_taken_branch_cost  */
  &ftc86x_advsimd_vector_cost, /* advsimd  */
  nullptr, /* sve  */
  nullptr /* issue_info  */
};
...
static const cpu_prefetch_tune ftc66x_prefetch_tune =
{
  8,                    /* num_slots  */
  32,                   /* l1_cache_size  */
  64,                   /* l1_cache_line_size  */
...
static const cpu_prefetch_tune ftc86x_prefetch_tune =
{
  8,                    /* num_slots  */
  64,                   /* l1_cache_size  */
  64,                   /* l1_cache_line_size  */
...
static const struct tune_params ftc66x_tunings =
{
  &ftc66x_extra_costs,
  &ftc66x_addrcost_table,
  &ftc66x_regmove_cost,
  &ftc66x_vector_cost,
  &generic_branch_cost,
  &generic_approx_modes,
  ...
  tune_params::AUTOPREFETCHER_WEAK,     /* autoprefetcher_model.  */
  (AARCH64_EXTRA_TUNE_NONE),  /* tune_flags.  */
  &ftc66x_prefetch_tune,
  AARCH64_LDP_STP_POLICY_ALWAYS,   /* ldp_policy_model.  */
  AARCH64_LDP_STP_POLICY_ALWAYS    /* stp_policy_model.  */
};

static const struct tune_params ftc86x_tunings =
{
  &ftc86x_extra_costs,
  &ftc86x_addrcost_table,
  &ftc86x_regmove_cost,
  &ftc86x_vector_cost,
  &generic_branch_cost,
  &generic_approx_modes,
  ...
  4,    /* issue_rate  */
  (AARCH64_FUSE_AES_AESMC | AARCH64_FUSE_ALU_CBZ | AARCH64_FUSE_CMP_BRANCH
   | AARCH64_FUSE_ALU_BRANCH), /* fusible_ops  */
  "16", /* function_align.  */
  "4",  /* jump_align.  */
  "8",  /* loop_align.  */
...
static void aarch64_parse_store_int_cost_string (const char*, struct tune_params*);
...
aarch64_tuning_override_functions[] =
{
  ...
  { "store_int_cost", aarch64_parse_store_int_cost_string },
  { NULL, NULL }
};
...
#define SLOT_REQUIRED     (-1)
  ...
  // disable CSR save/restore
  if (aarch64_flag_save_csr_by_ra)
    for (regno = R0_REGNUM; regno <= R30_REGNUM; regno++)
			   ...
			   bool hard_fp_valid_p)
{
  ...
  for (regno = aarch64_next_callee_save (start, limit);
		       ...
		       - cfun->machine->frame.reg_offset[regno]))
	{
	  ...
	  rtx cfi_mem2 = gen_frame_mem
	    (mode, plus_constant (Pmode, stack_pointer_rtx,
...
static bool
aarch64_is_csr_regno (unsigned regno)
{
  if (!aarch64_flag_save_csr_by_ra)
    return false;
...
static void
aarch64_copy_csr_to_virt (void)
{
  if (!aarch64_flag_save_csr_by_ra)
    return;

  typedef struct _save_pair
  {
    rtx hard_reg;
    rtx virt_reg;
  } save_pair;
  ...
  rtx_insn *seq;
  auto_vec<save_pair> sps;
  unsigned i;
  save_pair sp;
  save_pair *spi;

  // build CSR pair
  for (unsigned regno = R0_REGNUM; regno <= R30_REGNUM; regno++)
    {
      if (aarch64_is_csr_regno(regno))
	{
	  sp.hard_reg = gen_rtx_REG (Pmode, regno);
	  // build decl to mark virtual register prefer spill
	  tree var_decl = build_decl (UNKNOWN_LOCATION,
				      VAR_DECL,
				      get_identifier ("prefer_spill"),
				      void_type_node);
	  TREE_PUBLIC (var_decl) = 0;
	  DECL_ARTIFICIAL (var_decl) = 1;
	  SET_DECL_MODE (var_decl, Pmode);
	  DECL_IGNORED_P (var_decl) = 1;
	  // generate virtual register
	  sp.virt_reg = gen_reg_rtx (Pmode);
	  // set decl to virtual register
	  SET_DECL_RTL (var_decl, sp.virt_reg);
  ...
  // build save queue
  start_sequence ();
  FOR_EACH_VEC_ELT (sps, i, spi)
    {
      emit_move_insn (spi->virt_reg, spi->hard_reg);
    }
  seq = get_insns ();
  end_sequence ();
  // insert save queue on entry edge
  ...
  FOR_EACH_EDGE (e, ei, exit_bb->preds)
    {
      ...
      FOR_EACH_VEC_ELT(sps, i, spi)
	{
	  emit_move_insn (spi->hard_reg, spi->virt_reg);
	}
      seq = get_insns ();
      end_sequence ();

      // insert restore queue on exit edge
...
static void
aarch64_parse_store_int_cost_string (const char *tune_string,
				     struct tune_params *tune)
{
  int cost = 0;
  int n = sscanf (tune_string, "%d", &cost);
  if (n == EOF)
    {
      error ("invalid format for %<store_int_cost%>");
      return;
    }
  if (cost < 0)
    {
      error ("invalid %<store_int_cost%> value: %d", cost);
      return;
    }
  tune->memmov_cost.store_int = cost;
}
...
#undef TARGET_IS_CSR_REGNO
#define TARGET_IS_CSR_REGNO aarch64_is_csr_regno

#undef TARGET_COPY_CSR_TO_VIRT
#define TARGET_COPY_CSR_TO_VIRT aarch64_copy_csr_to_virt
