...
array_dse_callee::mult_tree (basic_block bb, tree t1, tree t2)
{
  ...
  if (wi::lt_p (range1.lower_bound (), 0, SIGNED))
    return nullptr;
...
array_dse_callee::calc_tree_range (basic_block bb, tree t)
{
  ...
  switch (TREE_CODE (t))
      ...
      case MINUS_EXPR:
	{
	  ...
	  auto len_min = len_range.lower_bound ().to_shwi ();
	  auto len_max = len_range.upper_bound ().to_shwi ();
			       ...
			       const value_range &r1, const value_range &r2)
{
  ...
  switch (op)
	...
	min = negate_tree (build_value (r1.upper_bound ().to_shwi ()));
	//max = negate_tree (r1.min ());
        max = negate_tree (build_value (r1.lower_bound ().to_shwi ()));
	break;
      case PLUS_EXPR:
	[[fallthrough]];
      case POINTER_PLUS_EXPR:
	//min = plus_tree (r1.min (), r2.min ());
        min = plus_tree (build_value (r1.lower_bound ().to_shwi ()), build_value (r2.lower_bound ().to_shwi ()));
	//max = plus_tree (r1.max (), r2.max ());
        max = plus_tree (build_value (r1.upper_bound ().to_shwi ()), build_value (r2.upper_bound ().to_shwi ()));
	break;
      case MINUS_EXPR:
	[[fallthrough]];
      case POINTER_DIFF_EXPR:
	//min = minus_tree (r1.min(), r2.max ());
        min = minus_tree (build_value (r1.lower_bound ().to_shwi ()), build_value (r2.upper_bound ().to_shwi ()));
	//max = minus_tree (r1.max(), r2.min ());
        max = minus_tree (build_value (r1.upper_bound ().to_shwi ()), build_value (r2.lower_bound ().to_shwi ()));
	break;
      case MULT_EXPR:
	//min = mult_tree (bb, r1.min (), r2.min ());
        min = mult_tree (bb, build_value (r1.lower_bound ().to_shwi ()), build_value (r2.lower_bound ().to_shwi ()));
	//max = mult_tree (bb, r1.max (), r2.max ());
        max = mult_tree (bb, build_value (r1.upper_bound ().to_shwi ()), build_value (r2.upper_bound ().to_shwi ()));
	break;
      case TRUNC_DIV_EXPR:
	//min = div_tree (bb, r1.min (), r2.max ());
        min = div_tree (bb, build_value (r1.lower_bound ().to_shwi ()), build_value (r2.upper_bound ().to_shwi ()));
	//max = div_tree (bb, r1.max (), r2.min ());
        max = div_tree (bb, build_value (r1.upper_bound ().to_shwi ()), build_value (r2.lower_bound ().to_shwi ()));
	break;
      case LSHIFT_EXPR:
	//min = lshift_tree (r1.min (), r2.min ());
        min = lshift_tree (build_value (r1.lower_bound ().to_shwi ()), build_value (r2.lower_bound ().to_shwi ()));
	//max = lshift_tree (r1.max (), r2.max ());
        max = lshift_tree (build_value (r1.upper_bound ().to_shwi ()), build_value (r2.upper_bound ().to_shwi ()));
	break;
      case RSHIFT_EXPR:
	//min = rshift_tree (r1.min (), r2.max ());
        min = rshift_tree (build_value (r1.lower_bound ().to_shwi ()), build_value (r2.upper_bound ().to_shwi ()));
	//max = rshift_tree (r1.max (), r2.min ());
        max = rshift_tree (build_value (r1.upper_bound ().to_shwi ()), build_value (r2.lower_bound ().to_shwi ()));
	break;
      case MAX_EXPR:
	//min = max_tree (bb, r1.min (), r2.min ());
        min = max_tree (bb, build_value (r1.lower_bound ().to_shwi ()), build_value (r2.lower_bound ().to_shwi ()));
	//max = max_tree (bb, r1.max (), r2.max ());
        max = max_tree (bb, build_value (r1.upper_bound ().to_shwi ()), build_value (r2.upper_bound ().to_shwi ()));
	break;
      case MIN_EXPR:
	//min = min_tree (bb, r1.min (), r2.min ());
        min = min_tree (bb, build_value (r1.lower_bound ().to_shwi ()), build_value (r2.lower_bound ().to_shwi ()));
	//max = min_tree (bb, r1.max (), r2.max ());
        max = min_tree (bb, build_value (r1.upper_bound ().to_shwi ()), build_value (r2.upper_bound ().to_shwi ()));
	break;
      default:
	break;
    }
...
array_dse_callee::compare_tree_by_minus (basic_block bb, tree t1, tree t2)
{
  ...
  HOST_WIDE_INT min = range.lower_bound ().to_shwi ();
  //HOST_WIDE_INT max = tree_to_shwi (range.max ());
  HOST_WIDE_INT max = range.upper_bound ().to_shwi ();
  if (min == 0)
    return GE;
  if (min > 0)
    return GT;
  ...
  if (gimple_code (stmt) == GIMPLE_PHI)
      ...
      for (unsigned i = 1; i < gimple_phi_num_args (stmt); i++)
	{
	  ...
	  //tree min = min_tree (bb, range.min (), arg_range.min ());
          tree min = min_tree (bb, build_value (range.lower_bound ().to_shwi ()), build_value (arg_range.lower_bound ().to_shwi ()));
	  //tree max = max_tree (bb, range.max (), arg_range.max ());
          tree max = max_tree (bb, build_value (range.upper_bound ().to_shwi ()), build_value (arg_range.upper_bound ().to_shwi ()));
	  if (!min || !max)
	    {
	      range.set_varying (RANGE_TYPE);
	      break;
	  ...
	  for (unsigned i = 0; i < gimple_num_ops (stmt); i++)
		...
		continue;
	      ...
	      //auto ret = compare_tree (bb, range.min (), integer_zero_node); 
              auto ret = compare_tree (bb, build_value (range.lower_bound ().to_shwi ()), integer_zero_node);
	      if (!ret || ret & LT)
		return false;
	      ...
	      //ret = compare_tree (bb, range.max (), tmp);
              ret = compare_tree (bb, build_value (range.upper_bound ().to_shwi ()), tmp);
	      if (!ret || ret & GT)
		return false;
	    }
	}
  ...
  for (unsigned i = 0; i < array_args.length (); i++)
    {
      ...
      auto ret = compare_tree (bb, build_value (len_range.lower_bound ().to_shwi ()), integer_one_node);
      if (!ret || ret & ~GE)
	return false;
      ...
      if (is_tail_recursive_call[i])
	{
	  ...
	  //ret = compare_tree (bb, array_range.min (), integer_zero_node);
          ret = compare_tree (bb, build_value (array_range.lower_bound ().to_shwi ()), integer_zero_node);
	  if (!ret || ret & ~GE)
	    return false;
	}
      else
	{
	  ...
	  //if (!integer_zerop (array_range.min ())
	    //  || !integer_zerop (array_range.max ()))
          if (!integer_zerop (build_value (array_range.lower_bound ().to_shwi ()))
              || !integer_zerop (build_value (array_range.upper_bound ().to_shwi ())))
	    return false;
	}
...
array_dse_callee::update_branch_range (basic_block bb)
{
  ...
  //tree min = range.min ();
  tree min = build_value (range.lower_bound ().to_shwi ());
  //tree max = range.max ();
  tree max = build_value (range.upper_bound ().to_shwi ());
  HOST_WIDE_INT value = TREE_INT_CST_LOW (rhs);
      ...
      if (TREE_CODE (rhs2) == SSA_NAME)
	{
	  ...
	  if (is_gimple_assign (stmt)
	      ...
	      && gimple_assign_rhs1 (stmt) == offset)
            recursive_ptr_max = build_value (get_var_range (bb, rhs1).upper_bound ().to_shwi ());
  ...
  if (!recursive_ptr_max)
    {
      ...
      recursive_ptr_max = plus_tree (build_value (range1.upper_bound ().to_shwi ()), build_value (range2.upper_bound ().to_shwi ()));
    }
...
array_dse_callee::calc_loop_var_range (loop_p loop)
{
  ...
  for (auto gsi = gsi_start_phis (header); !gsi_end_p (gsi); gsi_next (&gsi))
	...
	return false;
      ...
      tree min = step > 0 ? build_value (init_range.lower_bound ().to_shwi ()) : build_value (RANGE_NINF);
      //tree max = step > 0 ? build_value (RANGE_INF) : init_range.max ();
      tree max = step > 0 ? build_value (RANGE_INF) : build_value (init_range.upper_bound ().to_shwi ());
      ...
      for (auto bb : get_all_dominated_blocks (CDI_DOMINATORS, header))
	{
	  ...
	  //min = max_tree (bb, min, range.min ());
          min = max_tree (bb, min, build_value (range.lower_bound ().to_shwi ()));
	  //max = min_tree (bb, max, range.max ());
          max = min_tree (bb, max, build_value (range.upper_bound ().to_shwi ()));
	  if (!min || !max)
	    return false;
	  range = value_range {min, max};
	}
  ...
  for (auto bb : get_all_dominated_blocks (CDI_DOMINATORS, loop->header))
    {
      ...
      tree lhs_min = build_value (lhs_range.lower_bound ().to_shwi ());
      tree lhs_max = build_value (lhs_range.upper_bound ().to_shwi ());
      tree rhs_min = build_value (rhs_range.lower_bound ().to_shwi ());
      tree rhs_max = build_value (rhs_range.upper_bound ().to_shwi ());
...
array_dse_edge::calc_read_bound ()
{
  for (auto [var, stmt] : array_accesses)
	...
	continue;
      ...
      if (range.undefined_p() || range.varying_p()
	  || !integer_cst_p (build_value (range.upper_bound ().to_shwi ())))
	return false;

      //auto max = tree_to_shwi (range.max ());
      auto max = tree_to_shwi (build_value (range.upper_bound ().to_shwi ()));
      if (max % elem_size)
	return false;
...
array_dse_edge::calc_ref_range (tree var)
{
  ...
  if (code == ARRAY_REF)
	...
	return r;
      ...
      gcc_assert (integer_cst_p (build_value (r.lower_bound ().to_shwi ())) && integer_cst_p (build_value (r.upper_bound ().to_shwi ())));
      return make_range (tree_to_shwi (build_value (r.lower_bound ().to_shwi ())) * elem_size,
			 tree_to_shwi (build_value (r.upper_bound ().to_shwi ())) * elem_size);
    }
  ...
  return value_range{plus_tree (build_value (r1.lower_bound ().to_shwi ()), build_value (r2.lower_bound ().to_shwi ())),
		     plus_tree (build_value (r1.upper_bound ().to_shwi ()), build_value (r2.upper_bound ().to_shwi ()))};
}
  ...
  if (array_address_vars.contains (var))
      ...
      if (is_gimple_assign (stmt))
	{
	  ...
	  auto r2 = calc_offset_range (gimple_assign_rhs2 (stmt));
          //return value_range{plus_tree (r1.min (), r2.min ()),
                            // plus_tree (r1.max (), r2.max ())};
	  return value_range{plus_tree (build_value (r1.upper_bound ().to_shwi ()), build_value (r2.upper_bound ().to_shwi ())),
			     plus_tree (build_value (r1.upper_bound ().to_shwi ()), build_value (r2.upper_bound ().to_shwi ()))};
	}
...
array_dse_edge::calc_offset_range (tree offset)
{
  ...
  if (!integer_cst_p (build_value (range.lower_bound ().to_shwi ())) || !integer_cst_p (build_value (range.upper_bound ().to_shwi ())))
    return value_range{RANGE_TYPE};

  return make_range (tree_to_shwi (build_value (range.lower_bound ().to_shwi ())) * elem_size,
		     tree_to_shwi (build_value (range.upper_bound ().to_shwi ())) * elem_size);
}
...
array_dse_edge::calc_simple_loop_range (tree var)
{
  ...
  if (!init_range.singleton_p () || !integer_cst_p (build_value (init_range.lower_bound ().to_shwi ())))
    return value_range{RANGE_TYPE};

  //HOST_WIDE_INT init_value = tree_to_shwi (init_range.min ());
  HOST_WIDE_INT init_value = tree_to_shwi (build_value (init_range.lower_bound ().to_shwi ()));
...
array_dse_edge::calc_array_arg_start ()
{
  ...
  if (!range.singleton_p () || !integer_cst_p (build_value (range.lower_bound ().to_shwi ())))
    return false;

  //auto value = tree_to_shwi (range.min ());
  auto value = tree_to_shwi (build_value (range.lower_bound ().to_shwi ()));
  if (value % elem_size)
    return false;
...
array_dse_edge::check_optimized_area_rewrite ()
{
  ...
  len_arg_min = tree_to_shwi (build_value (range.lower_bound ().to_shwi ()));

  //return tree_to_shwi (range.max ()) + array_arg_start <= read_upper_bound;
  return tree_to_shwi (build_value (range.upper_bound ().to_shwi ())) + array_arg_start <= read_upper_bound;
}
