...
stmt_starts_throw_operation_p (gimple *stmt, gimple **throw_stmt_ptr = NULL)
{
  ...
  basic_block bb = gimple_bb (stmt);
  gimple *throw_stmt = *gsi_last_bb (bb);
...
function_wraps_delete_operator ()
{
  ...
  if (!simple)
  {
    ...
    gimple *cmp_stmt = *gsi_last_bb (cmp_bb);
    ...
    else if ((ret = safe_dyn_cast <greturn *> (*gsi_last_bb (skip_bb))))
    {
...
optimize_dynamic_cast (cgraph_edge *edge)
{
  ...
  if (*gsi_last_bb (call_bb) == call_stmt && !single_succ_p (call_bb))
    return false;
  ...
  gcond *cond_stmt = dyn_cast <gcond *> (*gsi_last_bb (call_bb));
  edge_def *e_cast;
  edge_def *e_fail;
