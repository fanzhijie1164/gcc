...
/* Return loop's size if it is complete unroll candidate,
   return -1 if can not do cunroll. */
static int
estimate_cunroll_loop_size (basic_block bb)
{
  if (!flag_cunroll_precheck)
    return -1;

  if (!loops_state_satisfies_p (cfun, LOOPS_NORMAL))
    return -1;

  /* Check bb is in a loop with constant iteration count. */
  class loop *caller_loop = bb->loop_father;
  if (!caller_loop || caller_loop->num == 0)
    return -1;

  HOST_WIDE_INT niter = get_max_loop_iterations_int (caller_loop);
  if (niter <= 0 || niter > param_max_completely_peel_times)
    return -1;

  /* Check if callee loop is a cunroll candidate. */
  basic_block *body = get_loop_body (caller_loop);
  int loop_size = 0;
  gimple_stmt_iterator gsi;
  for (unsigned i = 0; i < caller_loop->num_nodes; i++)
    for (gsi = gsi_start_bb (body[i]); !gsi_end_p (gsi); gsi_next (&gsi))
      {
	loop_size += estimate_num_insns (gsi_stmt (gsi), &eni_size_weights);
	if (loop_size > param_max_completely_peeled_insns)
	  return -1;
      }
	   ...
	   !gsi_end_p (bsi); gsi_next_nondebug (&bsi))
	{
	  ...
	  if (is_gimple_call (stmt)
		...
		if (TREE_CODE (gimple_call_arg (stmt, i)) == WITH_SIZE_EXPR)
	      ...
	      es->cunroll_candidate_loop_size =
		estimate_cunroll_loop_size (bb);
		       ...
		       bool prevails)
{
  ...
  int cunroll_loop_size = streamer_read_uhwi (ib);
  ...
  if (es)
    {
      ...
      es->loop_depth = depth;
      es->cunroll_candidate_loop_size = cunroll_loop_size;
    }
...
write_ipa_call_summary (struct output_block *ob, struct cgraph_edge *e)
{
  ...
  streamer_write_uhwi (ob, es->cunroll_candidate_loop_size);
