...
public:
  ...
  int size;
  /* Estimated size of the function after inner loops' cunroll. */
  int size_after_cunroll;

  ipa_size_summary ()
  : estimated_self_stack_size (0), self_size (0), size (0),
    size_after_cunroll(0)
  {
  }
};
...
public:
  ...
  ipa_call_summary ()
    : predicate (NULL), param (vNULL), call_stmt_size (0), call_stmt_time (0),
      loop_depth (0), cunroll_candidate_loop_size (0),
  ...
  ipa_call_summary (const ipa_call_summary &s):
    ...
    cunroll_candidate_loop_size (s.cunroll_candidate_loop_size),
  ...
  int cunroll_candidate_loop_size;
