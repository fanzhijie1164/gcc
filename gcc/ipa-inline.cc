...
/* Return true if inline may break loop complete unroll. */
static bool
may_break_loop_unroll_early (struct cgraph_edge *e, int growth)
{
  if (!flag_cunroll_precheck || growth <= param_max_completely_peeled_insns / 3)
    return false;
  ...
  gimple_seq phi_seq = phi_nodes (header);
  if (!phi_seq)
    return false;

  gimple_stmt_iterator psi = gsi_start (phi_seq);
  if (gsi_end_p (psi))
    return false;
  ...
  tree first_in = gimple_phi_arg_def (phi, 0);
  if (TREE_CODE(first_in) != INTEGER_CST)
  {
    if (TREE_CODE(first_in) != SSA_NAME)
      return false;

    gimple *first_in_stmt = SSA_NAME_DEF_STMT(first_in);
    if (!gimple_assign_single_p (first_in_stmt))
      return false;

    tree rhs = gimple_assign_rhs1 (first_in_stmt);
    if (TREE_CODE(rhs) != INTEGER_CST)
      return false;
  }

  /* Check the uses of phi: there should be exactly 2 non-DEBUG uses.
     One is in header exit (comparison with constant), the other is
     increment by 1 in another block. */
  imm_use_iterator use_iter;
  gimple *use_stmt;
  bool use_cond = false;
  bool use_plus = false;
  FOR_EACH_IMM_USE_STMT (use_stmt, use_iter, gimple_phi_result(phi))
    {
      /* Skip DEBUG statements */
      if (is_gimple_debug (use_stmt))
	continue;

      /* Check if this is the comparison in header exit */
      if (gimple_code (use_stmt) == GIMPLE_COND)
	{
	  if (use_cond)
	    return false;

	  tree op1 = gimple_cond_lhs (use_stmt);
	  tree op2 = gimple_cond_rhs (use_stmt);

	  /* Check if phi is compared with a constant */
	  if (TREE_CODE (op1) != INTEGER_CST
	      && TREE_CODE (op2) != INTEGER_CST)
	    return false;

	  /* Should be in header block */
	  if (gimple_bb (use_stmt) != header)
	    return false;

	  /* Check loop exit edge */
	  edge e0, e1;
	  extract_cond_bb_edges (header, &e0, &e1);
	  if (!loop_exit_edge_p (loop, e0) && !loop_exit_edge_p (loop, e1))
	    return false;
      ...
      /* Check if this is increment phi by 1 */
      if (gimple_code (use_stmt) == GIMPLE_ASSIGN)
	{
	  ...
	  if (!tree_fits_shwi_p (rhs2) || tree_to_shwi (rhs2) != 1)
	    return false;

	  tree second_in = gimple_phi_arg_def (phi, 1);
	  if (gimple_assign_lhs (use_stmt) != second_in)
	    return false;

	  use_plus = true;
	  continue;
	}
      /* If we get here, this is an unexpected use */
      return false;
    }
  if (!use_plus || !use_cond)
    return false;

  function *fn = e->caller->get_fun();
  int loop_size = 0;
  basic_block body;
  FOR_EACH_BB_FN (body, fn)
  {
    ...
    for (gsi = gsi_start_bb (body); !gsi_end_p (gsi); gsi_next (&gsi))
    {
      ...
      loop_size += estimate_num_insns (stmt, &eni_size_weights);
    }
    if (loop_size > param_max_completely_peeled_insns / 2)
      return false;
  }
  if (dump_enabled_p ())
    dump_printf_loc (MSG_MISSED_OPTIMIZATION, e->call_stmt,
		" caller: %C, callee:%C, "
		"loopsize: %d, growth: %d, threshold: %d\n",
		e->caller, e->callee, loop_size, growth,
		param_max_completely_peeled_insns);

  return ((loop_size + growth) > param_max_completely_peeled_insns / 2);
}

/* Return true if inline may break loop complete unroll. */
static bool
may_break_loop_unroll (struct cgraph_edge *e, int inline_growth)
{
  if (!flag_cunroll_precheck || inline_growth <= param_max_completely_peeled_insns / 3)
    return false;

  class ipa_call_summary *cs = ipa_call_summaries->get (e);
  int base_size = cs->cunroll_candidate_loop_size;
  if (base_size <= 0)
    return false;
  gcc_assert (base_size <= param_max_completely_peeled_insns);

  if ((base_size + inline_growth) <= (param_max_completely_peeled_insns / 2))
    return false;

  class ipa_size_summary *ss = ipa_size_summaries->get (e->callee);
  int size_benefit = ss->self_size + cs->call_stmt_size - inline_growth;
  if (size_benefit > param_inline_unroll_threshold)
    return false;
...
want_early_inline_function_p (struct cgraph_edge *e)
{
  ...
  /* For AutoFDO, we need to make sure that before profile summary, all
	       ...
	       && growth * (n + 1) > early_inlining_insns)
	{
      ...
      else if (may_break_loop_unroll_early (e, growth))
	{
	  if (dump_enabled_p ())
	    dump_printf_loc (MSG_MISSED_OPTIMIZATION, e->call_stmt,
			     "  will not early inline: %C->%C, "
			     "inline will break loop complete unroll\n",
			     e->caller, callee);
	  want_inline = false;
	}
    }
...
inline_small_functions (void)
{
  ...
  while (!edge_heap.empty ())
      ...
      if (!want_inline_small_function_p (edge, true))
	{
      ...
      if (may_break_loop_unroll (edge, growth))
	{
	  edge->inline_failed = CIF_INLINE_BREAK_CUNROLL;
	  report_inline_failed_reason (edge);
	  if (dump_file)
	    fprintf (dump_file,
		     "  will not inline: %s->%s, "
		     "inline will break loop complete unroll\n",
		     where->dump_name (), callee->dump_name ());
	  continue;
	}
