...
//split the structure into max 2 structures
...
struct srfunction
{
  ...
  srfunction *old;//the original function
  cgraph_node *newnode;
  srfunction *newf;//new function that originates from this function
...
private:
  //record the last escape reason
  escape_type escapes;

public:
  /* struct split: one struct with two fields transformed to two structures each with one field
     the new types this type was transformed to */
  tree newtype[max_split];
  //global ptr var/array for each allocated array
  tree pc_gptr;
  bool visited;
  //is pointer compression candidate
  bool pc_candidate;
  //known allocation num
  bool has_legal_alloc_num;
  /* Negative number means it has illegal allocated arrays
     that we do not optimize.
     Positive number means the number of allocation (calloc stmt) */
  int has_alloc_array;
  //the nth allocation
  int alloc_num;
  //need do semi-relayout
  bool semi_relayout;
  //<new field, its offset>
...
const unsigned READ_FIELD = 0x01u;//hexadecimal 1 unsigned
const unsigned WRITE_FIELD = 0x02u;

struct srfield
{
  unsigned HOST_WIDE_INT offset;
  tree fieldtype;//type of the field
  tree fielddecl;//original field decl
  srtype *base;//type of the struct to which the field belongs
  srtype *type;//if the ultimate type of this field is struct type, save, else null
  // = 1, means splitting the struct into two clusters
  unsigned clusternum;
  //newfield[2], new field decl for this field
...
struct srdecl
{
  ...
  */
  ...
  //whether the decl is ptrtoptr or not, null means not
  tree orig_type;
...
public:
  ...
  fc_field *input_field = NULL; //the original field of the shadow field must can be dynamic compressed.

  /* Init constants of the original srfield.  */
  tree init_const = NULL_TREE; //100000000 in unpair stmt
  ...
  tree input_var = NULL_TREE;//c

  /* The ssa for the input_var.  */
  tree input_ssa = NULL_TREE;//_199
