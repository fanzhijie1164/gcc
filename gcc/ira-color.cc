...
ira_pref_t
copy_prefs_linked_list(ira_allocno_t a,ira_allocno_t tmp)
{
        ira_pref_t newHead = NULL;
        ira_pref_t current = NULL;
        ira_pref_t prev = NULL;
        for (current = ALLOCNO_PREFS (a); current != NULL; current = current->next_pref)
        {
                ira_pref_t newNode = (ira_pref_t) ira_allocate (sizeof (struct ira_allocno_pref));
                newNode->num = current->num;
                newNode->hard_regno = current->hard_regno;
                newNode->freq = current->freq;
                newNode->allocno = tmp;
                newNode->next_pref = NULL;
                if (newHead == NULL)
                {
                        newHead = newNode;
                }
...
struct update_cost_record *
copy_update_cost_records_linked_list(struct update_cost_record *record)
{
        struct update_cost_record *newHead = NULL;
        struct update_cost_record *current = NULL;
        struct update_cost_record *prev = NULL;
        for(current = record; current != NULL; current = current->next)
        {
                struct update_cost_record *newNode = (struct update_cost_record *) ira_allocate (sizeof (struct update_cost_record));
                newNode->hard_regno = current->hard_regno;
                newNode->divisor = current->divisor;
                newNode->next = NULL;
                if (newHead == NULL)
                {
                        newHead = newNode;
...
static ira_allocno_t *new_ira_allocnos;
static ira_copy_t *new_ira_copies;
static ira_pref_t *new_ira_prefs;

static void
create_new_ira_copies(void)
{
        unsigned int i;
        ira_copy_t cp,tmp_cp;
        ira_copy_iterator ci;
        new_ira_copies = (ira_copy_t *) ira_allocate (sizeof (ira_copy_t) * ira_copies_num);

        FOR_EACH_COPY (cp, ci)
        {
                i = cp->num;
                ira_copy_t new_copy = (ira_copy_t) ira_allocate(sizeof (struct ira_allocno_copy));
                new_copy->num=cp->num;
                int first_num = ALLOCNO_NUM(cp->first);
                int second_num = ALLOCNO_NUM(cp->second);
                new_copy->first = new_ira_allocnos[first_num];
                new_copy->second = new_ira_allocnos[second_num];
                new_copy->freq = cp->freq;
                new_copy->constraint_p = cp->constraint_p;
                new_copy->insn = cp->insn; //CB may change it
                new_copy->loop_tree_node = cp->loop_tree_node; //CB may change it
                new_copy->prev_first_allocno_copy = NULL;
                new_copy->next_first_allocno_copy = NULL;
                new_copy->prev_second_allocno_copy = NULL;
                new_copy->next_second_allocno_copy = NULL;
                new_ira_copies[i] = new_copy;
        }
        ira_copy_t current_cp;
        FOR_EACH_COPY (cp, ci)
        {
                i=cp->num;
                current_cp = new_ira_copies[i];
                if (cp->prev_first_allocno_copy != NULL)
                {
                        tmp_cp = cp->prev_first_allocno_copy;
                        int prev_first_cp_num = tmp_cp->num;
                        current_cp->prev_first_allocno_copy = new_ira_copies[prev_first_cp_num];
                }
                if (cp->next_first_allocno_copy != NULL)
                {
                        tmp_cp = cp->next_first_allocno_copy;
                        int next_first_cp_num = tmp_cp->num;
                        current_cp->next_first_allocno_copy = new_ira_copies[next_first_cp_num];
                }
                if (cp->prev_second_allocno_copy != NULL)
                {
                        tmp_cp = cp->prev_second_allocno_copy;
                        int prev_second_cp_num = tmp_cp->num;
                        current_cp->prev_second_allocno_copy = new_ira_copies[prev_second_cp_num];
                }
                if (cp->next_second_allocno_copy != NULL)
                {
                        tmp_cp = cp->next_second_allocno_copy;
                        int next_second_cp_num = tmp_cp->num;
                        current_cp->next_second_allocno_copy = new_ira_copies[next_second_cp_num];
                }
        }
}

static void
create_new_ira_prefs(void)
{
        unsigned int i;

        ira_pref_t pref,tmp_pref;
        ira_pref_iterator pi;

        new_ira_prefs = (ira_pref_t *) ira_allocate (sizeof (ira_pref_t) * ira_prefs_num);

        FOR_EACH_PREF (pref, pi)
        {
                i = pref->num;
                ira_pref_t new_pref = (ira_pref_t) ira_allocate(sizeof (struct ira_allocno_pref));
                new_pref->num = pref->num;
                new_pref->hard_regno = pref->hard_regno;
                new_pref->freq = pref->freq;
                new_pref->allocno = new_ira_allocnos[ALLOCNO_NUM(pref->allocno)];
                new_pref->next_pref = NULL;
                new_ira_prefs[i] = new_pref;
        }
        ira_pref_t current_pref;
        FOR_EACH_PREF (pref, pi)
        {
                i=pref->num;
                current_pref = new_ira_prefs[i];
                if (pref->next_pref != NULL)
                {
                        tmp_pref = pref->next_pref;
                        int tmp_pref_num = tmp_pref->num;
                        current_pref->next_pref = new_ira_prefs[tmp_pref_num];
                }
        }
}
...
save_objects(void)
{
        ...
        {
                  int num = ALLOCNO_NUM(OBJECT_ALLOCNO(obj));
                  OBJECT_ALLOCNO (obj) = new_ira_allocnos[num];
                  OBJECT_SAVE_SUBWORD (obj) = OBJECT_SUBWORD (obj);
                  OBJECT_SAVE_CONFLICT_VEC_P (obj) = OBJECT_CONFLICT_VEC_P (obj);
                  OBJECT_SAVE_NUM_CONFLICTS (obj) = OBJECT_NUM_CONFLICTS (obj);
                  OBJECT_SAVE_CONFLICT_HARD_REGS (obj) = OBJECT_CONFLICT_HARD_REGS (obj);
                  OBJECT_SAVE_TOTAL_CONFLICT_HARD_REGS (obj) = OBJECT_TOTAL_CONFLICT_HARD_REGS (obj);
                  OBJECT_SAVE_MIN (obj) = OBJECT_MIN (obj);
                  OBJECT_SAVE_MAX (obj) = OBJECT_MAX (obj);
                  OBJECT_SAVE_CONFLICT_ID (obj) = OBJECT_CONFLICT_ID (obj);
        }
}

static void
restore_objects(void)
{
        ira_allocno_t tmp;
        ira_object_t obj;
        ira_object_iterator oi;
        FOR_EACH_OBJECT (obj, oi)
        {
                tmp = OBJECT_ALLOCNO(obj);
                OBJECT_ALLOCNO(obj) = OBJECT_SAVE_ALLOCNO(obj);
                OBJECT_SAVE_ALLOCNO(obj) = tmp;

                int tmp_subword;
                tmp_subword = OBJECT_SAVE_SUBWORD (obj);
                OBJECT_SAVE_SUBWORD (obj) = OBJECT_SUBWORD (obj);
                OBJECT_SUBWORD (obj) = tmp_subword;
                unsigned int tmp_conflict_vec_p;
                tmp_conflict_vec_p = OBJECT_SAVE_CONFLICT_VEC_P (obj);
                OBJECT_SAVE_CONFLICT_VEC_P (obj) = OBJECT_CONFLICT_VEC_P (obj);
                OBJECT_CONFLICT_VEC_P (obj) = tmp_conflict_vec_p;
                int tmp_num_accumulated_conflicts;
                tmp_num_accumulated_conflicts = OBJECT_SAVE_NUM_CONFLICTS (obj);
                OBJECT_SAVE_NUM_CONFLICTS (obj) = OBJECT_NUM_CONFLICTS (obj);
                OBJECT_NUM_CONFLICTS (obj) = tmp_num_accumulated_conflicts;

                HARD_REG_SET tmp_conflict_hard_regs,tmp_total_conflict_hard_regs;
                tmp_conflict_hard_regs = OBJECT_SAVE_CONFLICT_HARD_REGS (obj);
                OBJECT_SAVE_CONFLICT_HARD_REGS (obj) = OBJECT_CONFLICT_HARD_REGS (obj);
                OBJECT_CONFLICT_HARD_REGS (obj) = tmp_conflict_hard_regs;

                tmp_total_conflict_hard_regs = OBJECT_SAVE_TOTAL_CONFLICT_HARD_REGS (obj);
                OBJECT_SAVE_TOTAL_CONFLICT_HARD_REGS (obj) = OBJECT_TOTAL_CONFLICT_HARD_REGS (obj);
                OBJECT_TOTAL_CONFLICT_HARD_REGS (obj) = tmp_total_conflict_hard_regs;

                int tmp_min,tmp_max,tmp_id;
                tmp_min = OBJECT_SAVE_MIN (obj);
                OBJECT_SAVE_MIN (obj) = OBJECT_MIN (obj);
                OBJECT_MIN (obj) = tmp_min;

                tmp_max = OBJECT_SAVE_MAX (obj);
                OBJECT_SAVE_MAX (obj) = OBJECT_MAX (obj);
                OBJECT_MAX (obj) = tmp_max;

                tmp_id = OBJECT_SAVE_CONFLICT_ID (obj);
                OBJECT_SAVE_CONFLICT_ID (obj) = OBJECT_CONFLICT_ID (obj);
                OBJECT_CONFLICT_ID (obj) = tmp_id;
        }
}

static allocno_color_data_t
create_new_allocno_add_data(ira_allocno_t a)
{
        allocno_color_data_t data = ALLOCNO_COLOR_DATA (a);
        allocno_color_data_t new_data = (allocno_color_data_t) ira_allocate (sizeof (struct allocno_color_data));
        new_data->in_graph_p = data->in_graph_p;
        new_data->may_be_spilled_p = data->may_be_spilled_p;
        new_data->colorable_p = data->colorable_p;
        new_data->available_regs_num = data->available_regs_num;
        new_data->conflict_allocno_hard_prefs = data->conflict_allocno_hard_prefs;
        if (data->next_bucket_allocno != NULL)
        {
                int next_bucket_allocno_num = ALLOCNO_NUM(data->next_bucket_allocno);
                new_data->next_bucket_allocno = new_ira_allocnos[next_bucket_allocno_num];
        }
        else
                new_data->next_bucket_allocno = NULL;
        if (data->prev_bucket_allocno != NULL)
        {
                int prev_bucket_allocno_num = ALLOCNO_NUM(data->prev_bucket_allocno);
                new_data->prev_bucket_allocno = new_ira_allocnos[prev_bucket_allocno_num];
        }
        else
                new_data->prev_bucket_allocno = NULL;
        new_data->temp = data->temp;
        new_data->last_process = data->last_process;
        new_data->profitable_hard_regs = data->profitable_hard_regs;
        new_data->hard_regs_node = NULL; //need to think
        new_data->hard_regs_subnodes_start = data->hard_regs_subnodes_start;
        new_data->hard_regs_subnodes_num = data->hard_regs_subnodes_num;
        if (data->update_cost_records != NULL)
        {
                new_data->update_cost_records = copy_update_cost_records_linked_list(data->update_cost_records);
        }
        else
                new_data->update_cost_records = NULL;
        if (data->first_thread_allocno != NULL)
        {
                int first_thread_allocno_num = ALLOCNO_NUM(data->first_thread_allocno);
                new_data->first_thread_allocno = new_ira_allocnos[first_thread_allocno_num];
        }
        else
                new_data->first_thread_allocno = NULL;
        if (data->next_thread_allocno != NULL)
        {
                int next_thread_allocno_num = ALLOCNO_NUM(data->next_thread_allocno);
                new_data->next_thread_allocno = new_ira_allocnos[next_thread_allocno_num];
        }
        else
                new_data->next_thread_allocno = NULL;
        new_data->thread_freq = data->thread_freq;
        new_data->hard_reg_prefs = data->hard_reg_prefs;

        return new_data;
}

static void
add_new_ira_allocnos(void)
{
        unsigned int i,j;
        ira_allocno_t a,tmp;
        ...
        {
                ira_allocno_t new_allocno = (ira_allocno_t) ira_allocate (sizeof (struct ira_allocno));
                i = ALLOCNO_NUM(a);
                ALLOCNO_NUM(new_allocno) = ALLOCNO_NUM(a);
                ALLOCNO_REGNO(new_allocno) = ALLOCNO_REGNO(a);
                ALLOCNO_MODE(new_allocno) = ALLOCNO_MODE(a);
                ALLOCNO_WMODE(new_allocno) = ALLOCNO_WMODE(a);
                ALLOCNO_CLASS(new_allocno) = ALLOCNO_CLASS(a);
                aclass = ALLOCNO_CLASS (a);
                ALLOCNO_CROSSED_CALLS_ABIS(new_allocno) = ALLOCNO_CROSSED_CALLS_ABIS(a);
                ALLOCNO_DONT_REASSIGN_P(new_allocno) = ALLOCNO_DONT_REASSIGN_P(a);
                #ifdef STACK_REGS
                ALLOCNO_NO_STACK_REG_P(new_allocno) = ALLOCNO_NO_STACK_REG_P(a);
                ALLOCNO_TOTAL_NO_STACK_REG_P(new_allocno) = ALLOCNO_TOTAL_NO_STACK_REG_P(a);
                #endif
                ALLOCNO_BAD_SPILL_P(new_allocno) = ALLOCNO_BAD_SPILL_P(a);
                ALLOCNO_ASSIGNED_P(new_allocno) = ALLOCNO_ASSIGNED_P(a);

//              new_allocno->conflict_vec_p = a->conflict_vec_p;

                ALLOCNO_HARD_REGNO(new_allocno) = ALLOCNO_HARD_REGNO(a);

                ALLOCNO_NEXT_REGNO_ALLOCNO(new_allocno) = NULL;

                ALLOCNO_LOOP_TREE_NODE(new_allocno) = ALLOCNO_LOOP_TREE_NODE(a); //CB may change it

                ALLOCNO_NREFS(new_allocno) = ALLOCNO_NREFS(a);
                ALLOCNO_FREQ(new_allocno) = ALLOCNO_FREQ(a);
                ALLOCNO_CLASS_COST(new_allocno) = ALLOCNO_CLASS_COST(a);
                ALLOCNO_UPDATED_CLASS_COST(new_allocno) = ALLOCNO_UPDATED_CLASS_COST(a);
                ALLOCNO_MEMORY_COST(new_allocno) = ALLOCNO_MEMORY_COST(a);
                ALLOCNO_UPDATED_MEMORY_COST(new_allocno) = ALLOCNO_UPDATED_MEMORY_COST(a);
                ALLOCNO_EXCESS_PRESSURE_POINTS_NUM(new_allocno) = ALLOCNO_EXCESS_PRESSURE_POINTS_NUM(a);

                ALLOCNO_PREFS (new_allocno) = NULL;
                ALLOCNO_COPIES (new_allocno) = NULL;

                ALLOCNO_MIGHT_CONFLICT_WITH_PARENT_P (new_allocno) = ALLOCNO_MIGHT_CONFLICT_WITH_PARENT_P (a);

                ALLOCNO_CAP(new_allocno) = NULL;
                ALLOCNO_CAP_MEMBER(new_allocno) = NULL;

                ALLOCNO_NUM_OBJECTS(new_allocno) = ALLOCNO_NUM_OBJECTS(a);
                ALLOCNO_CALL_FREQ(new_allocno) = ALLOCNO_CALL_FREQ(a);
                ALLOCNO_CALLS_CROSSED_NUM(new_allocno) = ALLOCNO_CALLS_CROSSED_NUM(a);
                ALLOCNO_CHEAP_CALLS_CROSSED_NUM(new_allocno) = ALLOCNO_CHEAP_CALLS_CROSSED_NUM(a);
                ALLOCNO_CROSSED_CALLS_CLOBBERED_REGS(new_allocno) = ALLOCNO_CROSSED_CALLS_CLOBBERED_REGS(a);

                ALLOCNO_HARD_REG_COSTS (new_allocno) = NULL;
                ALLOCNO_UPDATED_HARD_REG_COSTS (new_allocno) = NULL;
                ALLOCNO_CONFLICT_HARD_REG_COSTS (new_allocno) = NULL;
                ALLOCNO_UPDATED_CONFLICT_HARD_REG_COSTS (new_allocno) = NULL;
                ALLOCNO_ADD_DATA (new_allocno) = NULL;
                //ALLOCNO_ADD_DATA (new_allocno) = ALLOCNO_ADD_DATA (a);//need to think
                new_ira_allocnos[i] = new_allocno;
        }
        create_new_ira_copies();
        create_new_ira_prefs();
        save_objects();
        FOR_EACH_ALLOCNO (a, ai)
        {
                i = ALLOCNO_NUM(a);
                tmp = new_ira_allocnos[i];
                aclass = ALLOCNO_CLASS (a);
                tmp->conflict_vec_p = a->conflict_vec_p;

                if (ALLOCNO_NEXT_REGNO_ALLOCNO(a)!=NULL)
                {
                        int tmp_num =ALLOCNO_NUM(ALLOCNO_NEXT_REGNO_ALLOCNO(a));
                        if(tmp_num > ira_allocnos_num)
                                ALLOCNO_NEXT_REGNO_ALLOCNO(tmp) = NULL;
                        else
                                ALLOCNO_NEXT_REGNO_ALLOCNO(tmp) = new_ira_allocnos[tmp_num];
                }
                if (ALLOCNO_PREFS (a)!=NULL)
                {
                        //ALLOCNO_PREFS (tmp) = copy_prefs_linked_list( a , tmp );
                        int pref_num = ALLOCNO_PREFS (a)->num;
                        ALLOCNO_PREFS (tmp) = new_ira_prefs[pref_num];
                }
                if (ALLOCNO_COPIES(a)!=NULL)
                {
                        int cp_num = ALLOCNO_COPIES(a)->num;
                        ALLOCNO_COPIES(tmp) = new_ira_copies[cp_num];
                }

                if (ALLOCNO_CAP(a)!=NULL)
                {
                        int tmp_cap = ALLOCNO_NUM(ALLOCNO_CAP(a));
                        ALLOCNO_CAP(tmp) = new_ira_allocnos[tmp_cap];
                }
                if (ALLOCNO_CAP_MEMBER(a)!=NULL)
                {
                        int tmp_cap_mem = ALLOCNO_NUM(ALLOCNO_CAP_MEMBER(a));
                        ALLOCNO_CAP_MEMBER(tmp) = new_ira_allocnos[tmp_cap_mem];
                }

                for (j = 0; j < ALLOCNO_NUM_OBJECTS(a);j++)
                {

                        obj = ALLOCNO_OBJECT (a, j);
                        ALLOCNO_OBJECT (tmp, j) = obj;
                }
                if (ALLOCNO_HARD_REG_COSTS (a) != NULL)
                {
                        ira_allocate_and_copy_costs
                                (&ALLOCNO_HARD_REG_COSTS (tmp),aclass,ALLOCNO_HARD_REG_COSTS (a));
                }
                if (ALLOCNO_UPDATED_HARD_REG_COSTS (a) != NULL)
                {
                        ira_allocate_and_copy_costs (&ALLOCNO_UPDATED_HARD_REG_COSTS (tmp),
                               aclass, ALLOCNO_UPDATED_HARD_REG_COSTS (a));
                }
                if (ALLOCNO_CONFLICT_HARD_REG_COSTS (a) != NULL)
                {
                        ira_allocate_and_copy_costs
                                (&ALLOCNO_CONFLICT_HARD_REG_COSTS (tmp), aclass,
                                ALLOCNO_CONFLICT_HARD_REG_COSTS (a));
                }
                if (ALLOCNO_UPDATED_CONFLICT_HARD_REG_COSTS (a) != NULL)
                {
                        ira_allocate_and_copy_costs
                                (&ALLOCNO_UPDATED_CONFLICT_HARD_REG_COSTS (tmp), aclass,
                                 ALLOCNO_UPDATED_CONFLICT_HARD_REG_COSTS (a));
                }

                //void *add_data;
                /*if (ALLOCNO_ADD_DATA(a) != NULL)
                {
                        ALLOCNO_ADD_DATA (tmp) = create_new_allocno_add_data(a);
                }*/
        }
        EXECUTE_IF_SET_IN_BITMAP (consideration_allocno_bitmap, 0, j, bi)
        {
                a = ira_allocnos[j];
                tmp = new_ira_allocnos[j];
                ALLOCNO_ADD_DATA (tmp) = create_new_allocno_add_data(a);
        }
}
...
static bool store_allocated_hardreg_p[FIRST_PSEUDO_REGISTER];
...
static ira_allocno_t *store_sorted_allocnos;
static int *store_allocno_priorities;
static int store_curr_allocno_process;
static int store_update_cost_check;
static ira_allocno_t store_update_cost_queue;
static struct update_cost_queue_elem *store_update_cost_queue_elems;
static struct update_cost_queue_elem *store_update_cost_queue_tail;
...
initiate_cost_update (void)
{
  ...
  update_cost_check = 0;
  store_update_cost_queue_elems
    = (struct update_cost_queue_elem *) ira_allocate (size);
  memset (store_update_cost_queue_elems, 0, size);
  store_update_cost_check = 0;
  /*
  store_update_cost_queue_tail = (struct update_cost_queue_elem *) ira_allocate (struct update_cost_queue_elem);
  memset (store_update_cost_queue_tail,0,sizeof (struct update_cost_queue_elem));
  */
}
...
finish_cost_update (void)
{
  ...
  ira_free (store_update_cost_queue_elems);
...
ira_soft_conflict (ira_allocno_t a1, ira_allocno_t a2)
{
  ...
  EXECUTE_IF_SET_IN_BITMAP (coloring_allocno_bitmap, 0, i, bi)
  {
        if (a1 == ira_allocnos[i])
        {
                flag = 1; //The input allocno 'a1' is from ira_allocnos.
                break;
        }
  }//If flag is still 0 , the input allocno 'a1' is from new_ira_allocnos.
  ...
  if (flag == 0)
    {
        num = ALLOCNO_NUM(local_a2);
        local_a2 = new_ira_allocnos[num];
    }
  ...
  for (;;)
    {
      ...
      if (flag == 0)
        {
          num = ALLOCNO_NUM(local_parent_a2);
          local_parent_a2 = new_ira_allocnos[num];
        }
  ...
  if (CHECKING_P)
      ...
      while (test_a2 != a2)
	{
	  test_a2 = ira_parent_allocno (test_a2);
          if (flag == 0)
            {
              num = ALLOCNO_NUM(test_a2);
              test_a2 = new_ira_allocnos[num];
            }
	  ira_assert (test_a2);
	}
    }
		      ...
		      HARD_REG_SET soft_conflict_regs, int hregno)
{
  ...
  EXECUTE_IF_SET_IN_BITMAP (coloring_allocno_bitmap, 0, i, bi)
  {
        if (a == ira_allocnos[i])
        {
                flag = 1; //The input allocno 'a' is from ira_allocnos.
                break;
        }
  }//If flag is still 0 , the input allocno 'a' is from new_ira_allocnos.
  ...
  EXECUTE_IF_SET_IN_BITMAP (allocnos_to_spill, 0, i, bi)
    {
	 ...
	 (not necessarily this one).  */
      ...
      if (flag == 0)
      {
        num = ALLOCNO_NUM(spill_a);
        spill_a = new_ira_allocnos[num];
      }
      ...
      do
	{
	  conflict_a = ira_parent_or_cap_allocno (conflict_a);
          if (flag == 0)
            {
              num = ALLOCNO_NUM(conflict_a);
              conflict_a = new_ira_allocnos[num];
            }
...
static void
store_global_variables(void) //store_...:last loop's result, after restore for CB
{
        int i;
        for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
        {
                store_allocated_hardreg_p[i] = allocated_hardreg_p[i];
        }

        for (i = 0; i < ira_allocnos_num; i++)
        {
                store_sorted_allocnos[i] = sorted_allocnos[i];
        }
        memcpy (store_allocno_priorities, allocno_priorities, sizeof (int) * ira_allocnos_num);
        store_curr_allocno_process = curr_allocno_process;
        store_update_cost_check = update_cost_check;
        if (update_cost_queue != NULL && (ALLOCNO_NUM(update_cost_queue) < ira_allocnos_num))
        {
                int tmp_num = ALLOCNO_NUM(update_cost_queue);
                store_update_cost_queue = update_cost_queue; //need to debug to check
                update_cost_queue = new_ira_allocnos[tmp_num];
        }
        else
                store_update_cost_queue = NULL;
        for (i = 0; i < ira_allocnos_num;i++)
        {
                        store_update_cost_queue_elems[i].check = update_cost_queue_elems[i].check;
                        store_update_cost_queue_elems[i].divisor = update_cost_queue_elems[i].divisor;
                        if (update_cost_queue_elems[i].start != NULL)
                        {
                                store_update_cost_queue_elems[i].start = update_cost_queue_elems[i].start;
                                int tmp_num = ALLOCNO_NUM(update_cost_queue_elems[i].start);
                                update_cost_queue_elems[i].start = new_ira_allocnos[tmp_num];
                        }
                        else
                                store_update_cost_queue_elems[i].start =  NULL;
                        if (update_cost_queue_elems[i].from != NULL)
                        {
                                store_update_cost_queue_elems[i].from = update_cost_queue_elems[i].from;
                                int tmp_num = ALLOCNO_NUM(update_cost_queue_elems[i].from);
                                update_cost_queue_elems[i].from = new_ira_allocnos[tmp_num];
                        }
                        else
                                store_update_cost_queue_elems[i].from = NULL;
                        if (update_cost_queue_elems[i].next != NULL)
                        {
                                store_update_cost_queue_elems[i].next = update_cost_queue_elems[i].next;
                                int tmp_num = ALLOCNO_NUM(update_cost_queue_elems[i].next);
                                update_cost_queue_elems[i].next = new_ira_allocnos[tmp_num];
                        }
                        else
                                store_update_cost_queue_elems[i].next = NULL;
         }
        int index;
        if (update_cost_queue_tail != NULL)
        {
                for (i = 0; i < ira_allocnos_num;i++)
                {
                        if(update_cost_queue_tail == &update_cost_queue_elems[i])
                        {
                                index = i;
                                store_update_cost_queue_tail = &store_update_cost_queue_elems[index];
                        }
                }
        }
        else
                store_update_cost_queue_tail = NULL;
}

static void
restore_global_variables(void)  //store_...:priority's result
{
        int i;
        bool tmp_allocated_hardreg_p;
        ira_allocno_t a;

        for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
        {
                tmp_allocated_hardreg_p = allocated_hardreg_p[i];
                allocated_hardreg_p[i] = store_allocated_hardreg_p[i];
                store_allocated_hardreg_p[i] = tmp_allocated_hardreg_p;
        }
        for (i = 0; i < ira_allocnos_num;i++)
        {
                a = sorted_allocnos[i];
                sorted_allocnos[i] = store_sorted_allocnos[i];
                store_sorted_allocnos[i] = a;
        }
        int *tmp_allocno_priorities = (int *) ira_allocate (sizeof (int) * ira_allocnos_num);
        memcpy (tmp_allocno_priorities, allocno_priorities, sizeof (int) * ira_allocnos_num);
        memcpy (allocno_priorities, store_allocno_priorities, sizeof (int) * ira_allocnos_num);
        memcpy (store_allocno_priorities, tmp_allocno_priorities, sizeof (int) * ira_allocnos_num);
        ira_free (tmp_allocno_priorities);

        int tmp_curr_allocno_process;
        tmp_curr_allocno_process = curr_allocno_process;
        curr_allocno_process = store_curr_allocno_process;
        store_curr_allocno_process = tmp_curr_allocno_process;

        int tmp_update_cost_check;
        tmp_update_cost_check = update_cost_check;
        update_cost_check = store_update_cost_check;
        store_update_cost_check = tmp_update_cost_check;

        ira_allocno_t tmp_update_cost_queue;
        tmp_update_cost_queue = update_cost_queue;
        update_cost_queue = store_update_cost_queue;
        store_update_cost_queue = tmp_update_cost_queue;
        for (i = 0; i < ira_allocnos_num;i++)
        {
                        int tmp_check;
                        tmp_check = store_update_cost_queue_elems[i].check;
                        store_update_cost_queue_elems[i].check = update_cost_queue_elems[i].check;
                        update_cost_queue_elems[i].check = tmp_check;

                        int tmp_divisor;
                        tmp_divisor = store_update_cost_queue_elems[i].divisor;
                        store_update_cost_queue_elems[i].divisor = update_cost_queue_elems[i].divisor;
                        update_cost_queue_elems[i].divisor = tmp_divisor;

                        ira_allocno_t tmp;
                        tmp = store_update_cost_queue_elems[i].start;
                        store_update_cost_queue_elems[i].start = update_cost_queue_elems[i].start;
                        update_cost_queue_elems[i].start = tmp;

                        tmp = store_update_cost_queue_elems[i].from;
                        store_update_cost_queue_elems[i].from = update_cost_queue_elems[i].from;
                        update_cost_queue_elems[i].from = tmp;

                        tmp = store_update_cost_queue_elems[i].next;
                        store_update_cost_queue_elems[i].next = update_cost_queue_elems[i].next;
                        update_cost_queue_elems[i].next = tmp;
        }

        int index,store_index;
        struct update_cost_queue_elem *tmp_elem;
        tmp_elem = store_update_cost_queue_tail;
        int flag = 0;
        for  (i = 0; i < ira_allocnos_num;i++)
        {
                if (update_cost_queue_tail == &update_cost_queue_elems[i])
        ...
        for  (i = 0; i < ira_allocnos_num;i++)
        {
                if (tmp_elem == &store_update_cost_queue_elems[i])
                {
                        store_index = i;
                        flag = 1;
                        update_cost_queue_tail = &update_cost_queue_elems[store_index];
                }
        }
        if (flag == 0)
        {
                update_cost_queue_tail = NULL;
        }
}

/* Replace current results with priority's results*/
static void
replace_new_ira_copies(void)
{
        int i;
        ira_copy_t cp,curr_cp,tmp_cp;
        ira_copy_iterator ci;
        FOR_EACH_COPY (cp, ci)
        {
                i = cp->num;
                curr_cp = new_ira_copies[i];
                cp->num = curr_cp->num;
                /*int first_num = ALLOCNO_NUM(curr_cp->first);
                int second_num = ALLOCNO_NUM(curr_cp->second);
                cp->first = ira_allocnos[first_num];
                cp->second = ira_allocnos[second_num];*/
                cp->freq = curr_cp->freq;
                cp->constraint_p = curr_cp->constraint_p;
                cp->insn = curr_cp->insn;
                cp->loop_tree_node = curr_cp->loop_tree_node;
                if (curr_cp->prev_first_allocno_copy != NULL)
                {
                        tmp_cp = curr_cp->prev_first_allocno_copy;
                        int prev_first_cp_num = tmp_cp->num;
                        cp->prev_first_allocno_copy = ira_copies[prev_first_cp_num];
                }
                else
                        cp->prev_first_allocno_copy = NULL;
                if (curr_cp->next_first_allocno_copy != NULL)
                {
                        tmp_cp = curr_cp->next_first_allocno_copy;
                        int next_first_cp_num = tmp_cp->num;
                        cp->next_first_allocno_copy = ira_copies[next_first_cp_num];
                }
                else
                        cp->next_first_allocno_copy = NULL;
                if (curr_cp->prev_second_allocno_copy != NULL)
                {
                        tmp_cp = curr_cp->prev_second_allocno_copy;
                        int prev_second_cp_num = tmp_cp->num;
                        cp->prev_second_allocno_copy = ira_copies[prev_second_cp_num];
                }
                else
                        cp->prev_second_allocno_copy = NULL;
                if (curr_cp->next_second_allocno_copy != NULL)
                {
                        tmp_cp = curr_cp->next_second_allocno_copy;
                        int next_second_cp_num = tmp_cp->num;
                        cp->next_second_allocno_copy = ira_copies[next_second_cp_num];
                }
                else
                        cp->next_second_allocno_copy = NULL;
        }
}

static void
replace_new_ira_prefs(void)
{
        int i;
        ira_pref_t pref,current_pref,tmp_pref;
        ira_pref_iterator pi;
        FOR_EACH_PREF (pref, pi)
        {
                i = pref->num;
                current_pref = new_ira_prefs[i];
                pref->num = current_pref->num;
                pref->hard_regno = current_pref->hard_regno;
                pref->freq = current_pref->freq;
                //pref->allocno = ira_allocnos[ALLOCNO_NUM(current_pref->allocno)];
                if (current_pref->next_pref != NULL)
                {
                        tmp_pref = current_pref->next_pref;
                        int tmp_pref_num = tmp_pref->num;
                        pref->next_pref = ira_prefs[tmp_pref_num];
                }
                else
                        pref->next_pref = NULL;
        }
...
static allocno_color_data_t
replace_new_allocno_add_data(ira_allocno_t a,ira_allocno_t curr)
{
        allocno_color_data_t curr_data = ALLOCNO_COLOR_DATA (curr);
        allocno_color_data_t data = ALLOCNO_COLOR_DATA (a);
        data->in_graph_p = curr_data->in_graph_p;
        data->may_be_spilled_p = curr_data->may_be_spilled_p;
        data->colorable_p = curr_data->colorable_p;
        data->available_regs_num = curr_data->available_regs_num;
        data->conflict_allocno_hard_prefs = curr_data->conflict_allocno_hard_prefs;
        if (curr_data->next_bucket_allocno != NULL)
        {
                int next_bucket_allocno_num = ALLOCNO_NUM(curr_data->next_bucket_allocno);
                data->next_bucket_allocno = ira_allocnos[next_bucket_allocno_num];
        }
        else
                data->next_bucket_allocno = NULL;
        if (curr_data->prev_bucket_allocno != NULL)
        {
                int prev_bucket_allocno_num = ALLOCNO_NUM(curr_data->prev_bucket_allocno);
                data->prev_bucket_allocno = ira_allocnos[prev_bucket_allocno_num];
        }
        else
                data->prev_bucket_allocno = NULL;
        data->temp = curr_data->temp;
        data->last_process = curr_data->last_process;
        data->profitable_hard_regs = curr_data->profitable_hard_regs;
        data->hard_regs_node = NULL; //need to think
        data->hard_regs_subnodes_start = curr_data->hard_regs_subnodes_start;
        data->hard_regs_subnodes_num = curr_data->hard_regs_subnodes_num;
        //update_cost_records:create struct update_cost_record *,need free or not ?
        if (curr_data->update_cost_records != NULL)
        {
                data->update_cost_records = copy_update_cost_records_linked_list(curr_data->update_cost_records);
        }
        else
                data->update_cost_records = NULL;
        if (curr_data->first_thread_allocno != NULL)
        {
                int first_thread_allocno_num = ALLOCNO_NUM(curr_data->first_thread_allocno);
                data->first_thread_allocno = ira_allocnos[first_thread_allocno_num];
        }
        else
                data->first_thread_allocno = NULL;
        if (curr_data->next_thread_allocno != NULL)
        {
                int next_thread_allocno_num = ALLOCNO_NUM(curr_data->next_thread_allocno);
                data->next_thread_allocno = ira_allocnos[next_thread_allocno_num];
        }
        else
                data->next_thread_allocno = NULL;
        data->thread_freq = curr_data->thread_freq;
        data->hard_reg_prefs = curr_data->hard_reg_prefs;

        return data;
}

static void
replace_all_with_priority_results(void)
{
        unsigned int i,j;
        bitmap_iterator bi;
        ira_allocno_t a,tmp;
        ...
        //EXECUTE_IF_SET_IN_BITMAP (consideration_allocno_bitmap, 0, i, bi)
        {
                ...
                //a = ira_allocnos[i];
                tmp = new_ira_allocnos[i];
                ALLOCNO_NUM(a) = ALLOCNO_NUM(tmp);
                ALLOCNO_REGNO(a) = ALLOCNO_REGNO(tmp);
                ALLOCNO_MODE(a) = ALLOCNO_MODE(tmp);
                ALLOCNO_WMODE(a) = ALLOCNO_WMODE(tmp);
                ALLOCNO_CLASS(a) = ALLOCNO_CLASS(tmp);
                aclass = ALLOCNO_CLASS (a);
                ALLOCNO_CROSSED_CALLS_ABIS(a) = ALLOCNO_CROSSED_CALLS_ABIS(tmp);
                ALLOCNO_DONT_REASSIGN_P(a) = ALLOCNO_DONT_REASSIGN_P(tmp);
                #ifdef STACK_REGS
                ALLOCNO_NO_STACK_REG_P(a) = ALLOCNO_NO_STACK_REG_P(tmp);
                ALLOCNO_TOTAL_NO_STACK_REG_P(a) = ALLOCNO_TOTAL_NO_STACK_REG_P(tmp);
                #endif
                ALLOCNO_BAD_SPILL_P(a) = ALLOCNO_BAD_SPILL_P(tmp);
                ALLOCNO_ASSIGNED_P(a) = ALLOCNO_ASSIGNED_P(tmp);
                a->conflict_vec_p = tmp->conflict_vec_p;
                ALLOCNO_HARD_REGNO(a) = ALLOCNO_HARD_REGNO(tmp);


                if (ALLOCNO_NEXT_REGNO_ALLOCNO(tmp) != NULL)
                {
                        int tmp_num = ALLOCNO_NUM(ALLOCNO_NEXT_REGNO_ALLOCNO(tmp));
                        ALLOCNO_NEXT_REGNO_ALLOCNO(a) = ira_allocnos[tmp_num];
                }
                else
                        ALLOCNO_NEXT_REGNO_ALLOCNO(a) = NULL;
                ALLOCNO_LOOP_TREE_NODE(a) = ALLOCNO_LOOP_TREE_NODE(tmp);
                ALLOCNO_NREFS(a) = ALLOCNO_NREFS(tmp);
                ALLOCNO_FREQ(a) = ALLOCNO_FREQ(tmp);
                ALLOCNO_CLASS_COST(a) = ALLOCNO_CLASS_COST(tmp);
                ALLOCNO_UPDATED_CLASS_COST(a) = ALLOCNO_UPDATED_CLASS_COST(tmp);
                ALLOCNO_MEMORY_COST(a) = ALLOCNO_MEMORY_COST(tmp);
                ALLOCNO_EXCESS_PRESSURE_POINTS_NUM(a) = ALLOCNO_EXCESS_PRESSURE_POINTS_NUM(tmp);
                ALLOCNO_MIGHT_CONFLICT_WITH_PARENT_P (a) = ALLOCNO_MIGHT_CONFLICT_WITH_PARENT_P (tmp);
                if (ALLOCNO_CAP(tmp) != NULL)
                {
                        int tmp_cap = ALLOCNO_NUM(ALLOCNO_CAP(tmp));
                        ALLOCNO_CAP(a) = ira_allocnos[tmp_cap];
                }
                else
                        ALLOCNO_CAP(a) = NULL;
                if (ALLOCNO_CAP_MEMBER(tmp) != NULL)
                {
                        int tmp_cap_mem = ALLOCNO_NUM(ALLOCNO_CAP_MEMBER(tmp));
                        ALLOCNO_CAP_MEMBER(a) = ira_allocnos[tmp_cap_mem];
                }
                else
                        ALLOCNO_CAP_MEMBER(a) = NULL;
                ALLOCNO_NUM_OBJECTS(a) = ALLOCNO_NUM_OBJECTS(tmp);
                ALLOCNO_CALL_FREQ(a) = ALLOCNO_CALL_FREQ(tmp);
                ALLOCNO_CALLS_CROSSED_NUM(a) = ALLOCNO_CALLS_CROSSED_NUM(tmp);
                ALLOCNO_CHEAP_CALLS_CROSSED_NUM(a) = ALLOCNO_CHEAP_CALLS_CROSSED_NUM(tmp);
                ALLOCNO_CROSSED_CALLS_CLOBBERED_REGS(a) = ALLOCNO_CROSSED_CALLS_CLOBBERED_REGS(tmp);
                int len;
                len = ira_class_hard_regs_num[aclass];

                if (ALLOCNO_HARD_REG_COSTS (tmp) != NULL)
                        memcpy (ALLOCNO_HARD_REG_COSTS (a), ALLOCNO_HARD_REG_COSTS (tmp), sizeof (int) * len);
                else
                        ALLOCNO_HARD_REG_COSTS (a) = NULL;

                if (ALLOCNO_UPDATED_HARD_REG_COSTS (tmp) != NULL)
                        memcpy (ALLOCNO_UPDATED_HARD_REG_COSTS (a), ALLOCNO_UPDATED_HARD_REG_COSTS (tmp), sizeof (int) * len);
                else
                        ALLOCNO_UPDATED_HARD_REG_COSTS (a) = NULL;

                if (ALLOCNO_CONFLICT_HARD_REG_COSTS (tmp) != NULL)
                        memcpy (ALLOCNO_CONFLICT_HARD_REG_COSTS (a), ALLOCNO_CONFLICT_HARD_REG_COSTS (tmp), sizeof (int) * len);
                else
                        ALLOCNO_CONFLICT_HARD_REG_COSTS (a) = NULL;

                if (ALLOCNO_UPDATED_CONFLICT_HARD_REG_COSTS (tmp) != NULL)
                        memcpy (ALLOCNO_UPDATED_CONFLICT_HARD_REG_COSTS (a), ALLOCNO_UPDATED_CONFLICT_HARD_REG_COSTS (tmp), sizeof (int) * len);
                else
                        ALLOCNO_UPDATED_CONFLICT_HARD_REG_COSTS (a) = NULL;
                //ALLOCNO_ADD_DATA (a) = ALLOCNO_ADD_DATA (tmp);
        }
        replace_new_ira_copies();
        replace_new_ira_prefs();
        //EXECUTE_IF_SET_IN_BITMAP (consideration_allocno_bitmap, 0, i, bi)
        FOR_EACH_ALLOCNO (a, ai)
        {
                //a = ira_allocnos[i];
                ...
                tmp = new_ira_allocnos[i];
                aclass = ALLOCNO_CLASS (a);
                if (ALLOCNO_PREFS(tmp) != NULL)
                {
                        int pref_num = ALLOCNO_PREFS(tmp)->num;
                        ALLOCNO_PREFS(a) = ira_prefs[pref_num];
                }
                else
                        ALLOCNO_PREFS(a) = NULL;
                if (ALLOCNO_COPIES(tmp) != NULL)
                {
                        int cp_num = ALLOCNO_COPIES(tmp)->num;
                        ALLOCNO_COPIES(a) = ira_copies[cp_num];
                }
                else
                        ALLOCNO_COPIES(a) = NULL;
                if (ALLOCNO_COLOR_DATA(tmp) != NULL)
                {
                        // ALLOCNO_ADD_DATA (a) = replace_new_allocno_add_data(a,tmp);
                        continue;
                }
                else
...
static void
replace_objects_with_priority_results(void)
{
        ira_object_t obj;
        ira_object_iterator oi;
        FOR_EACH_OBJECT (obj, oi)
        {
                /*int num = ALLOCNO_NUM(OBJECT_SAVE_ALLOCNO(obj));
                OBJECT_ALLOCNO(obj) = ira_allocnos[num];*/
                OBJECT_SAVE_ALLOCNO(obj) = OBJECT_ALLOCNO(obj);
                OBJECT_SUBWORD (obj) = OBJECT_SAVE_SUBWORD (obj);
                OBJECT_CONFLICT_VEC_P (obj) = OBJECT_SAVE_CONFLICT_VEC_P (obj);
                OBJECT_NUM_CONFLICTS (obj) = OBJECT_SAVE_NUM_CONFLICTS (obj);
                OBJECT_CONFLICT_HARD_REGS (obj) = OBJECT_SAVE_CONFLICT_HARD_REGS (obj);
                OBJECT_TOTAL_CONFLICT_HARD_REGS (obj) = OBJECT_SAVE_TOTAL_CONFLICT_HARD_REGS (obj);
                OBJECT_MIN (obj) = OBJECT_SAVE_MIN (obj);
                OBJECT_MAX (obj) = OBJECT_SAVE_MAX (obj);
                OBJECT_CONFLICT_ID (obj) = OBJECT_SAVE_CONFLICT_ID (obj);
        }
}

static void
replace_objects_with_cb_results(void)
{
        ira_object_t obj;
        ira_object_iterator oi;
        FOR_EACH_OBJECT (obj, oi)
...
static void
replace_global_variables(int n)
{
        int i,num;
        ...
        memset (store_allocated_hardreg_p, 0, sizeof (store_allocated_hardreg_p));

        for (i = 0; i < n;i++)
        {
                num = ALLOCNO_NUM(store_sorted_allocnos[i]);
                sorted_allocnos[i] = ira_allocnos[num];
        }
        memcpy (allocno_priorities, store_allocno_priorities, sizeof (int) * ira_allocnos_num);

        curr_allocno_process = store_curr_allocno_process;
        update_cost_check = store_update_cost_check;
        if (store_update_cost_queue != NULL && (ALLOCNO_NUM(store_update_cost_queue) < ira_allocnos_num))        {
                num = ALLOCNO_NUM(store_update_cost_queue);
                update_cost_queue = ira_allocnos[num];
        }
        else
                update_cost_queue = NULL;
        for (i = 0; i < ira_allocnos_num;i++)
        {
                update_cost_queue_elems[i].check = store_update_cost_queue_elems[i].check;
                update_cost_queue_elems[i].divisor = store_update_cost_queue_elems[i].divisor;
                if (store_update_cost_queue_elems[i].start != NULL)
                {
                        num = ALLOCNO_NUM(store_update_cost_queue_elems[i].start);
                        update_cost_queue_elems[i].start = ira_allocnos[num];
                }
                if (store_update_cost_queue_elems[i].from != NULL)
                {
                        num = ALLOCNO_NUM(store_update_cost_queue_elems[i].from);
                        update_cost_queue_elems[i].from = ira_allocnos[num];
                }
                if (store_update_cost_queue_elems[i].next != NULL)
                {
                        num = ALLOCNO_NUM(store_update_cost_queue_elems[i].next);
                        update_cost_queue_elems[i].next = ira_allocnos[num];
                }
        }
        int index;
        if (store_update_cost_queue_tail != NULL)
        {
                for (i = 0; i < ira_allocnos_num;i++)
                {
                        if(store_update_cost_queue_tail == &store_update_cost_queue_elems[i])
                        {
                                index = i;
                                update_cost_queue_tail = &update_cost_queue_elems[index];
                        }
                }
        }
        else
...
/*Free all structures which are created for both algorithm.*/
static void
free_new_ira_copies(void)
{
        int i;
        ira_copy_t cp,tmp_cp;
        ira_copy_iterator ci;
        ...
        ira_free (new_ira_copies);
        new_ira_copies = NULL;
}

static void
free_new_ira_prefs(void)
{
        int i;
        ira_pref_t pref,tmp_pref;
        ira_pref_iterator pi;
        ...
        ira_free (new_ira_prefs);
        new_ira_prefs = NULL;
}

static void
free_new_ira_allocnos(void)
{
        free_new_ira_copies();
        free_new_ira_prefs();
        ...
        EXECUTE_IF_SET_IN_BITMAP (consideration_allocno_bitmap, 0, j, bi)
        {
                ...
                if (tmp != NULL)
                {
                        if (ALLOCNO_ADD_DATA(tmp) != NULL)
                        {
                                if (ALLOCNO_COLOR_DATA(tmp)->update_cost_records != NULL)
                                {
                                        struct update_cost_record *record = ALLOCNO_COLOR_DATA(tmp)->update_cost_records;
                                        while((record->next) != NULL)
                                        {
                                                struct update_cost_record *tmp_cost = record;
                                                record = record->next;
                                                ira_free(tmp_cost);
                                                tmp_cost = NULL;
                                        }
                                ...
                                ira_free(ALLOCNO_COLOR_DATA(tmp));
                                //ira_free(ALLOCNO_ADD_DATA (tmp));
                                //ALLOCNO_COLOR_DATA(tmp) = NULL;
                                ALLOCNO_ADD_DATA (tmp) = NULL;
                        }
                }
        }
        ...
        ira_free (new_ira_allocnos);
        new_ira_allocnos = NULL;
}

/*Calculate single algorithm's overall/reg/mem cost.*/
int64_t p_ira_overall_cost,p_ira_reg_cost,p_ira_mem_cost;
int64_t cb_ira_overall_cost,cb_ira_reg_cost,cb_ira_mem_cost;

static void
p_calculate_allocation_cost (void)
{
        int hard_regno, cost;
        ira_allocno_t a;
        p_ira_overall_cost=p_ira_reg_cost=p_ira_mem_cost=0;
        unsigned int i;
        bitmap_iterator bi;

        EXECUTE_IF_SET_IN_BITMAP (coloring_allocno_bitmap, 0, i, bi)
        {
                a = new_ira_allocnos[i];
                hard_regno = ALLOCNO_HARD_REGNO (a);
                ira_assert (hard_regno < 0 || (ira_hard_reg_in_set_p
                ...
                p_ira_overall_cost += cost;
        }
}

static void
cb_calculate_allocation_cost (void)
{
        int hard_regno, cost;
        ira_allocno_t a;
        cb_ira_overall_cost=cb_ira_reg_cost=cb_ira_mem_cost=0;
        unsigned int i;
        bitmap_iterator bi;
        EXECUTE_IF_SET_IN_BITMAP (coloring_allocno_bitmap, 0, i, bi)
        {
...
color_allocnos (int loop_num)
{
  ...
  int cb_spill_num = 0;
  int p_spill_num = 0;
  int p_qsort_assign_mem_num = 0;
  int cb_pop_spill_num = 0;
  int sum_p = 0;
  int sum_cb = 0;
  ...
  if (flag_ira_algorithm == IRA_ALGORITHM_BOTH)
  {
       add_new_ira_allocnos();
       ...
       if (n != 0)
        ...
        cb_pop_spill_num = pop_allocnos_from_stack ();
        finish_allocno_hard_regs_nodes_forest ();
        cb_calculate_allocation_cost();

       if (internal_flag_ira_verbose > 3 && ira_dump_file != NULL)
        {
                fprintf (ira_dump_file,"\nFunction name:%s  Loop %d.",
                                        current_function_name(),loop_num);
                fprintf (ira_dump_file,"\nThe priority and CB algorithm have done.\nResults:\n");
                fprintf (ira_dump_file,"p_spill_num:%d ,p_qsort_assign_mem_num:%d \n",
                                        p_spill_num,p_qsort_assign_mem_num);
                fprintf (ira_dump_file,"cb_spill_num:%d ,cb_pop_spill_num:%d \n",
                                        cb_spill_num,cb_pop_spill_num);
        }
       if (internal_flag_ira_verbose > 0 && ira_dump_file != NULL)
        {
                fprintf (ira_dump_file,
                                "\n+++Loop Priority Costs: overall %" PRId64
                                ", reg %" PRId64
                                ", mem %" PRId64,
                                p_ira_overall_cost, p_ira_reg_cost, p_ira_mem_cost);
                fprintf (ira_dump_file,
                                "\n+++Loop CB Costs: overall %" PRId64
                                ", reg %" PRId64
                                ", mem %" PRId64,
                                cb_ira_overall_cost, cb_ira_reg_cost, cb_ira_mem_cost);
        }
        sum_p = p_spill_num + p_qsort_assign_mem_num;
        sum_cb = cb_spill_num + cb_pop_spill_num;

       /*
        //Test priority:
        if (internal_flag_ira_verbose > 3 && ira_dump_file != NULL)
        {
                fprintf (ira_dump_file,"\nChoose Priority Algorithm.\n\n");
        }
        replace_all_with_priority_results();
        replace_objects_with_priority_results();
        replace_global_variables(n);
        */
        ...
        replace_objects_with_cb_results();
        */
       ...
       //Test both:
        ...
        {
                if(p_ira_overall_cost < cb_ira_overall_cost)
                {
                        if (internal_flag_ira_verbose > 3 && ira_dump_file != NULL)
                        {
                                fprintf (ira_dump_file,"\nChoose Priority Algorithm.\n\n");
...
color_pass (ira_loop_tree_node_t loop_tree_node)
{
  ...
  color_allocnos (loop_tree_node->loop_num);
...
ira_initiate_assign (void)
{
  ...
  sorted_copies = (ira_copy_t *) ira_allocate (ira_copies_num
					       * sizeof (ira_copy_t));
  store_sorted_allocnos
          = (ira_allocno_t *) ira_allocate (sizeof (ira_allocno_t)* ira_allocnos_num);
  store_allocno_priorities = (int *) ira_allocate (sizeof (int) * ira_allocnos_num);
}
...
ira_finish_assign (void)
{
  ...
  ira_free (store_sorted_allocnos);
  ira_free (store_allocno_priorities);
}
