...
scan_one_insn (rtx_insn *insn)
{
  ...
  if (set != 0 && REG_P (SET_DEST (set)))
    {
      rtx dest = SET_DEST (set);
      if (targetm.is_csr_regno(REGNO (dest)))
	{
	  csr_in_dest = true;
	}
    }
      ...
      if (REG_P (op) && REGNO (op) >= FIRST_PSEUDO_REGISTER)
	{
	  ...
	  for (k = cost_classes_ptr->num - 1; k >= 0; k--)
		...
		p_costs[k] += add_cost;
	      ...
	      if (csr_in_dest && cost_classes_ptr->classes[k] == GENERAL_REGS)
		{
		  tree decl = REG_EXPR (op);
		  if (decl != NULL_TREE
		      && VAR_P (decl)
		      && DECL_NAME (decl)
		      && TREE_CODE (DECL_NAME (decl)) == IDENTIFIER_NODE
		      && id_equal (DECL_NAME (decl), "prefer_spill"))
		    {
		      p->mem_cost = p_costs[k];
		    }
		}
