...
struct ira_object
{
  ...
  /* The allocno associated with this record.(backup)  */
  ira_allocno_t save_allocno;
  int save_subword;
  //unsigned int save_conflicts_array_size;
  int save_id;
  int save_min, save_max;
  HARD_REG_SET save_conflict_hard_regs, save_total_conflict_hard_regs;
  int save_num_accumulated_conflicts;
  unsigned int save_conflict_vec_p : 1;
};
...
#define OBJECT_SAVE_ALLOCNO(O) ((O)->save_allocno)
#define OBJECT_SAVE_SUBWORD(O) ((O)->save_subword)
#define OBJECT_SAVE_CONFLICT_VEC_P(O) ((O)->save_conflict_vec_p)
#define OBJECT_SAVE_NUM_CONFLICTS(O) ((O)->save_num_accumulated_conflicts)
#define OBJECT_SAVE_CONFLICT_HARD_REGS(O) ((O)->save_conflict_hard_regs)
#define OBJECT_SAVE_TOTAL_CONFLICT_HARD_REGS(O) ((O)->save_total_conflict_hard_regs)
#define OBJECT_SAVE_MIN(O) ((O)->save_min)
#define OBJECT_SAVE_MAX(O) ((O)->save_max)
#define OBJECT_SAVE_CONFLICT_ID(O) ((O)->save_id)
