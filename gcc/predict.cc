...
//Determine whether expr is directly an array or memory access
static bool
expr_is_mem_or_array_access(tree expr)
{
    if (!expr)
        return false;
    ...
    switch (TREE_CODE(expr))
    {
        ...
        case SSA_NAME:
        {
            ...
            switch (gimple_code(def_stmt))
            {
                case GIMPLE_ASSIGN:
                {
                    ...
                    return expr_is_mem_or_array_access(rhs);
                }
                default:
                    return false;
            }
        ...
        case CONVERT_EXPR:
            return expr_is_mem_or_array_access(TREE_OPERAND(expr, 0));
...
//Strict judgment condition: true is returned only when EQ_EXPR is used and memory/array access is relied upon
static bool
cond_is_eq_mem_or_array_access(gimple *stmt)
{
    if (!stmt || gimple_code(stmt) != GIMPLE_COND)
        return false;

    enum tree_code cmp = gimple_cond_code(stmt);
    if (cmp != EQ_EXPR)
        return false;
    ...
    return expr_is_mem_or_array_access(op0) || expr_is_mem_or_array_access(op1);
}

//Determine whether expr is a struct field access
static bool
expr_is_component_ref(tree expr)
{
    if (!expr)
        return false;
    ...
    switch (TREE_CODE(expr))
    {
        ...
        case SSA_NAME:
        {
            ...
            switch (gimple_code(def_stmt))
            {
                ...
                {
                    tree rhs = gimple_assign_rhs1(def_stmt);
                    return expr_is_component_ref(rhs);
                }
                default:
                    return false;
            }
        ...
        case CONVERT_EXPR:
            return expr_is_component_ref(TREE_OPERAND(expr, 0));
...
//Check the conditional statement: Both are struct fields on the left and right, and they are not equal
static bool
cond_is_ne_component_ref(gimple *stmt)
{
    if (!stmt || gimple_code(stmt) != GIMPLE_COND)
        return false;
    ...
    return expr_is_component_ref(op0) && expr_is_component_ref(op1);
}

static bool
is_adjacent_relational_condition(gimple *cur, gimple *next)
{

    if (!cur || gimple_code(cur) != GIMPLE_COND)
      return false;

    if (!next || gimple_code(next) != GIMPLE_COND)
      return false;

    enum tree_code cur_code = gimple_cond_code(cur);
    enum tree_code next_code = gimple_cond_code(next);

    tree cur_op0 = gimple_cond_lhs(cur);
    tree cur_op1 = gimple_cond_rhs(cur);

    tree next_op0 = gimple_cond_lhs(next);
    tree next_op1 = gimple_cond_rhs(next);

    if ( TREE_CODE( cur_op0 ) == INTEGER_CST || TREE_CODE( cur_op1 ) == INTEGER_CST ) return false;
    if ( cur_op0 != next_op0 || cur_op1 != next_op1 ) return false; 

    return (cur_code == LT_EXPR && next_code == GT_EXPR);
}

static bool
is_loop_relational_condition(gimple *cur, basic_block cur_bb){

    if (!cur || gimple_code(cur) != GIMPLE_COND)
      return false;

    if(!loop_depth (cur_bb->loop_father))
      return false;

    tree cur_op1 = gimple_cond_rhs(cur);
    if ( TREE_CODE( cur_op1 ) != INTEGER_CST ) 
      return false;

    enum tree_code cur_code = gimple_cond_code(cur);
    return (cur_code == GT_EXPR);
}
...
tree_predict_by_opcode (basic_block bb)
{
  ...
  if( flag_decouple_mem_branches ){

      gimple_stmt_iterator gsi = gsi_last_bb (bb->prev_bb);

      gimple *stmt_prev = gsi_stmt (gsi);

      if( cond_is_eq_mem_or_array_access(stmt) && cond_is_ne_component_ref(stmt_prev) ){

          predict_edge (then_edge, PRED_BUILTIN_EXPECT, HITRATE (5));

          if( dump_file )
              fprintf(dump_file, "modify bb probability: %i\n", bb->index);
      }

  }

  if( flag_improve_relational_branches ){

      basic_block next_bb = bb->next_bb->next_bb;
    
      if (next_bb)
      {
        gimple *stmt_next = *gsi_last_nondebug_bb(next_bb);
        if (!stmt_next) {
          /* If the block is empty or has no statement, skip.  */
        } else if (is_adjacent_relational_condition(stmt, stmt_next))
        {
            /* Use param relational-branches1-probability instead of hardcoded 10 */
            predict_edge (then_edge, PRED_BUILTIN_EXPECT, 
                          HITRATE (param_relational_branches1_probability));
  ...
  if( flag_improve_relational_branches ){
    ...
    if (is_loop_relational_condition(stmt, bb)){

       /* Use param relational-branches2-probability instead of hardcoded 10 */
       predict_edge (then_edge, PRED_BUILTIN_EXPECT, 
                     HITRATE (param_relational_branches2_probability));
