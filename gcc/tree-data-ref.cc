...
get_extended_segment_min_max (const dr_with_seg_len &d, tree *seg_min_out,
                     tree *seg_max_out, HOST_WIDE_INT align, class loop *loop)
{
    ...
    tree init_ref = build_fold_indirect_ref(init_addr);
        ...
        /* Analyze the behavior of memory reference D.DR within DR_STMT for the outer loop */
        struct innermost_loop_behavior innermost;
    opt_result res = dr_analyze_innermost(&innermost, init_ref, loop_outer(loop), DR_STMT(d.dr));

    if(!res)
      return false;
    /* addr_base = base_address + offset + init */
    tree addr_base = fold_build_pointer_plus (innermost.base_address, innermost.offset);
    addr_base = fold_build_pointer_plus (addr_base, innermost.init);
    /* seg_len_for_outer_loop = inner_loop_niters * outer_loop_niters */
    tree seg_len_for_outer_loop = d.seg_len_for_outer_loop;
    if(seg_len_for_outer_loop == NULL_TREE)
      return false;
    tree seg_len
        = fold_convert (sizetype, rewrite_to_non_trapping_overflow (d.seg_len_for_outer_loop));
			       ...
			       const dr_with_seg_len_pair_t &alias_pair)
{
  ...
  *cond_expr = NULL_TREE;
  ...
  /* If hoist_runtime_checks flag being specified,
     doing some early check to ensure outer loop exist,
     and niters is integral for both inner and outer loop,
     if early checking passed, avoid generating ifn_alias_check
         and waw_or_war_check. */
  bool hoist_runtime_checks = flag_hoist_runtime_checks;
  if (hoist_runtime_checks){
        /* Check if inner/outer loop meeting the basic condition of hoisting */
    class loop* outer_loop = loop_outer(loop);
    tree outer_loop_niters = number_of_latch_executions (outer_loop);
    tree inner_loop_niters = number_of_latch_executions (loop);
    const_tree inner_type;
    if(inner_loop_niters)
      inner_type = TREE_TYPE(inner_loop_niters);
    const_tree outer_type;
    if(outer_loop_niters)
      outer_type = TREE_TYPE(outer_loop_niters);
    if (/* Loop num equals to non-zero implies loop exist */
        (outer_loop && outer_loop->num)
        && (loop && loop->num)
        && outer_type && inner_type
        /* Check integral to insure seg length computable */
        && INTEGRAL_TYPE_P(inner_type) && INTEGRAL_TYPE_P(outer_type)
                && int_binop_types_match_p (MULT_EXPR, outer_type, inner_type))
      hoist_runtime_checks = true;
    else
      hoist_runtime_checks = false;
  }

  /* If runtime check possibly happen, do not create diff check */
  if (!hoist_runtime_checks){
  ...
  tree seg_a_min_old = seg_a_min;
  tree seg_a_max_old = seg_a_max;
  tree seg_b_min_old = seg_b_min;
  tree seg_b_max_old = seg_b_max;
  class loop *outer_loop = loop_outer(loop);
  /* if hoist flag and both two dataref's start address are variant relative
     to the outer loop, we can consider hoisting the runtime memory check out
     of outer loop */
  if(hoist_runtime_checks
     && !expr_invariant_in_loop_p(outer_loop, seg_a_min)
     && !expr_invariant_in_loop_p(outer_loop, seg_b_min))
  {
     bool succ_a = get_extended_segment_min_max (dr_a, &seg_a_min, &seg_a_max, min_align, loop);
     bool succ_b = get_extended_segment_min_max (dr_b, &seg_b_min, &seg_b_max, min_align, loop);
     if(succ_a && succ_b){
         if (dump_enabled_p ())
            dump_printf (MSG_NOTE, "Inner Expand the range of address to include those by the outer loop.\n");
     }else{ /* Recover the address relative to inner loop if range expanding failed */
         seg_a_min = seg_a_min_old;
         seg_a_max = seg_a_max_old;
         seg_b_min = seg_b_min_old;
         seg_b_max = seg_b_max_old;
         hoist_runtime_checks = false;
     }
  }
