...
/* Return true if LOOP is a structurally plausible candidate for hoisting
   runtime alias checks to its immediate outer loop.  This is only a cheap
   precheck; individual data-reference pairs require further analysis.  */

bool
runtime_check_hoisting_precheck_p (class loop *loop)
{
  if (!flag_hoist_runtime_checks || !loop || !loop->num)
    return false;
  ...
  if (!outer_loop_niters)
    return false;

  const_tree outer_type = TREE_TYPE (outer_loop_niters);
  return INTEGRAL_TYPE_P (outer_type);
}
			  ...
			  const dr_with_seg_len_pair_t &alias_pair)
{
  ...
  tree subject = build2 (MINUS_EXPR, sizetype,
...
/* Calculate the extended segment min and max address relative to the
   outer loop, instead of the inner loop.
...
static bool
get_extended_segment_min_max (const dr_with_seg_len &d, tree *seg_min_out,
			      tree *seg_max_out, HOST_WIDE_INT align,
			      class loop *loop)
{
  if (!d.outer_loop_range_p)
    return false;

  tree base = unshare_expr (DR_BASE_ADDRESS (d.dr));
  tree offset = unshare_expr (DR_OFFSET (d.dr));
  tree init = unshare_expr (DR_INIT (d.dr));
  tree init_offset = fold_build2 (PLUS_EXPR, TREE_TYPE (offset), init, offset);
  tree init_addr = fold_build_pointer_plus (base, init_offset);
  tree init_ref = build_fold_indirect_ref (init_addr);

  /* Analyze the behavior of memory reference D.DR within DR_STMT for the
     outer loop.  */
  struct innermost_loop_behavior innermost;
  opt_result res = dr_analyze_innermost (&innermost, init_ref,
					 loop_outer (loop), DR_STMT (d.dr));
  if (!res)
    return false;

  tree outer_loop_niters = number_of_latch_executions (loop_outer (loop));
  if (!outer_loop_niters)
    return false;

  tree inner_indicator = dr_direction_indicator (d.dr);
  tree inner_neg_step = fold_build2 (LT_EXPR, boolean_type_node,
				     fold_convert (ssizetype, inner_indicator),
				     ssize_int (0));
  tree inner_seg_len
    = fold_convert (sizetype, rewrite_to_non_trapping_overflow (d.seg_len));
  tree inner_min_reach = fold_build3 (COND_EXPR, sizetype, inner_neg_step,
				      inner_seg_len, size_zero_node);
  tree inner_max_reach = fold_build3 (COND_EXPR, sizetype, inner_neg_step,
				      size_zero_node, inner_seg_len);

  tree outer_neg_step = fold_build2 (LT_EXPR, boolean_type_node,
				     fold_convert (ssizetype, innermost.step),
				     ssize_int (0));
  tree outer_step
    = fold_convert (sizetype, rewrite_to_non_trapping_overflow (innermost.step));
  tree outer_niters
    = fold_convert (sizetype,
		    rewrite_to_non_trapping_overflow (outer_loop_niters));
  tree outer_reach = fold_build2 (MULT_EXPR, sizetype,
				  outer_step, outer_niters);
  tree outer_min_reach = fold_build3 (COND_EXPR, sizetype, outer_neg_step,
				      outer_reach, size_zero_node);
  tree outer_max_reach = fold_build3 (COND_EXPR, sizetype, outer_neg_step,
				      size_zero_node, outer_reach);

  tree min_reach = fold_build2 (PLUS_EXPR, sizetype,
				outer_min_reach, inner_min_reach);
  tree max_reach = fold_build2 (PLUS_EXPR, sizetype,
				outer_max_reach, inner_max_reach);
  max_reach = fold_build2 (PLUS_EXPR, sizetype, max_reach,
			   size_int (d.access_size - align));

  tree addr_base = fold_build_pointer_plus (innermost.base_address,
					    innermost.offset);
  addr_base = fold_build_pointer_plus (addr_base, innermost.init);
...
/* Return true if ALIAS_PAIR can be represented by address bounds that are
   invariant in the immediate outer loop of LOOP.  */

bool
hoistable_runtime_alias_pair_p (class loop *loop,
				const dr_with_seg_len_pair_t &alias_pair)
{
  if (!runtime_check_hoisting_precheck_p (loop))
    return false;

  if (alias_pair.flags & (DR_ALIAS_ARBITRARY | DR_ALIAS_MIXED_STEPS))
    return false;
  ...
  const dr_with_seg_len &dr_a = alias_pair.first;
  const dr_with_seg_len &dr_b = alias_pair.second;
  if (!dr_a.outer_loop_range_p || !dr_b.outer_loop_range_p)
    return false;

  if (TREE_CODE (DR_STEP (dr_a.dr)) != INTEGER_CST
      || !tree_fits_shwi_p (DR_STEP (dr_a.dr))
      || TREE_CODE (DR_STEP (dr_b.dr)) != INTEGER_CST
      || !tree_fits_shwi_p (DR_STEP (dr_b.dr)))
    return false;

  tree seg_a_min, seg_a_max, seg_b_min, seg_b_max;
  unsigned int min_align = MIN (dr_a.align, dr_b.align);
  if (!get_extended_segment_min_max (dr_a, &seg_a_min, &seg_a_max,
				     min_align, loop))
    return false;

  if (!get_extended_segment_min_max (dr_b, &seg_b_min, &seg_b_max,
				     min_align, loop))
    return false;

  class loop *outer_loop = loop_outer (loop);
  return (expr_invariant_in_loop_p (outer_loop, seg_a_min)
	  && expr_invariant_in_loop_p (outer_loop, seg_a_max)
	  && expr_invariant_in_loop_p (outer_loop, seg_b_min)
	  && expr_invariant_in_loop_p (outer_loop, seg_b_max));
}
...
create_intersect_range_checks (class loop *loop, tree *cond_expr,
			       const dr_with_seg_len_pair_t &alias_pair,
				   bool widen_alias_checks,
				   bool use_outer_index_check)
{
  const dr_with_seg_len &dr_a = alias_pair.first;
  const dr_with_seg_len &dr_b = alias_pair.second;
  *cond_expr = NULL_TREE;

  if ((!widen_alias_checks || use_outer_index_check)
      && create_intersect_range_checks_index (loop, cond_expr, alias_pair,
					      true))
    return;
  ...
  /* If runtime memory check hoisting possibly happen,
     do not create waw or war checks */
  if (!widen_alias_checks)
    {
  ...
  if (widen_alias_checks)
    {
      bool succ_a = get_extended_segment_min_max (dr_a, &seg_a_min,
						  &seg_a_max, min_align, loop);
      bool succ_b = get_extended_segment_min_max (dr_b, &seg_b_min,
						  &seg_b_max, min_align, loop);
      if (!succ_a || !succ_b)
	gcc_unreachable ();

      if (dump_enabled_p ())
	dump_printf (MSG_NOTE,
		     "expanded the alias check range to cover the outer loop.\n");
    }
...
create_runtime_alias_checks (class loop *loop,
			     ...
			     tree *cond_expr,
			     bool allow_outer_loop_versioning)
{
  tree part_cond_expr;
  fold_defer_overflow_warnings ();

  bool widen_alias_checks
    = (allow_outer_loop_versioning
	&& runtime_check_hoisting_precheck_p (loop));
  auto_vec<unsigned char> use_outer_index_check;

  if (widen_alias_checks)
    {
      use_outer_index_check.safe_grow_cleared (alias_pairs->length ());
      unsigned int i;
      dr_with_seg_len_pair_t *alias_pair;
      FOR_EACH_VEC_ELT (*alias_pairs, i, alias_pair)
	{
	  tree index_cond = NULL_TREE;
	  if (create_intersect_range_checks_index (loop, &index_cond,
						     *alias_pair, false)
	      && expr_invariant_in_loop_p (loop_outer (loop), index_cond))
	    use_outer_index_check[i] = 1;
	  else if (!hoistable_runtime_alias_pair_p (loop, *alias_pair))
	    {
	      widen_alias_checks = false;
	      break;
	    }
	}

      if (!widen_alias_checks)
	use_outer_index_check.release ();
    }
  ...
  FOR_EACH_VEC_ELT (*alias_pairs, i, alias_pair)
	...
	(loop, &part_cond_expr, *alias_pair, widen_alias_checks,
	 widen_alias_checks && use_outer_index_check[i]);
      if (*cond_expr)
	*cond_expr = fold_build2 (TRUTH_AND_EXPR, boolean_type_node,
				  *cond_expr, part_cond_expr);
      else
