...
public:
  dr_with_seg_len (data_reference_p d, tree len, unsigned HOST_WIDE_INT size,
                   unsigned int a)
    : dr (d), seg_len (len), seg_len2 (len), outer_loop_range_p (false), access_size (size), align (a)
    {}
  dr_with_seg_len (data_reference_p d, tree len, tree len2, 
                   unsigned HOST_WIDE_INT size, unsigned int a)
    : dr (d), seg_len (len), seg_len2 (len2), outer_loop_range_p (true), access_size (size), align (a)
    {}
  ...
  tree seg_len2;
  /* True if the pair was produced by the vectorizer and can therefore be
     considered for an outer-loop range check.  */
  bool outer_loop_range_p;
  /* The segment length for immediate outer loop, which is used to do
  runtime memory checking for outer loop after the check being hoisted out of outer loop. */
  tree seg_len_for_outer_loop;
