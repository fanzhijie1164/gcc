...
{
  /* When loop nodes' number is 2, there is no need to check the latch */
  if (loop->num_nodes == 2)
    return true;
...
determine_loop_form (class loop *loop)
{
  ...
  if (loop->inner || (loop->num_nodes != 3 && loop->num_nodes != 2))
    {
      if (dump_file && (dump_flags & TDF_DETAILS))
	{
