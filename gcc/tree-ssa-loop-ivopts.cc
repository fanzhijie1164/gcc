...
/* Count SSA names in 't' defined in outside 'loop'.  */

static int
count_outer_ssa_names (struct loop *loop, tree t, int depth)
{
  /* Prevent stack overflow on deep trees */
  if (depth <= 0 || !t)
    return 0;
  ...
  switch (TREE_CODE (t))
            ...
            if (bb && !flow_bb_inside_loop_p (loop, bb))
              {
                ...
                if (dump_file && (dump_flags & TDF_DETAILS))
                  {
                    fprintf(dump_file, "      [Count] Outer SSA: ");
    ...
    /* Handle all binary arithmetic ops */
    case PLUS_EXPR: 
    case MINUS_EXPR: 
    case MULT_EXPR: 
    case TRUNC_DIV_EXPR: 
    ...
    case COND_EXPR:
      {
        if (TREE_OPERAND_LENGTH (t) >= 1)
          count += count_outer_ssa_names (loop, TREE_OPERAND (t, 0), depth - 1);
        if (TREE_OPERAND_LENGTH (t) >= 2)
          count += count_outer_ssa_names (loop, TREE_OPERAND (t, 1), depth - 1);
      }
      break;

    /* NOP_EXPR is the most common one for implicit casts like (int)var */
    case NOP_EXPR:
    case CONVERT_EXPR:
    case VIEW_CONVERT_EXPR:
    ...
    default:
      /* For constants, addresses, etc., stop recursion */
      break;
    }
...
determine_iv_cost (struct ivopts_data *data, struct iv_cand *cand)
{
  ...
  /* Penalize candidates whose base depends on more than two outer loop variables.
     This prevents register pressure explosion in deeply nested loops.  */
  int penalize_cost = param_penalize_unprofitable_ivcands_cost;
  if (penalize_cost > 0 && cand->pos != IP_ORIGINAL && base)
    {
      int outer_var_count = count_outer_ssa_names (data->current_loop, base, 100);
      
      /* If base depends on more than 2 outer variables, add a significant penalty.  
         A penalty of 15 ensures it's rejected unless absolutely critical. */
      if (outer_var_count > 1)
        {
          int penalty = outer_var_count * penalize_cost; 
          cost += penalty;
          
          if (dump_file && (dump_flags & TDF_DETAILS))
            fprintf (dump_file, "  Penalizing candidate %u: %d outer vars, penalty +%d\n",
                     cand->id, outer_var_count, penalty);
        }
    }
...
/* Helper function: Determine if an SSA_NAME belongs to a pure 
   dead computation chain (used to filter out remnants left by IVOPTS) */
static bool
is_dead_computation_chain_p (tree name, auto_vec<tree> *visited)
{
  ...
  for (unsigned i = 0; i < visited->length (); i++)
    if ((*visited)[i] == name)
      return true; 
  visited->safe_push (name);

  gimple *def = SSA_NAME_DEF_STMT (name);
  /* If the definition statement has side effects (e.g., memory Store, function call), 
     it is not dead code  */
  if (def && gimple_has_side_effects (def))
    return false;

  /* If there are no uses, it is obviously dead code */
  ...
  FOR_EACH_IMM_USE_FAST (use_p, iter, name)
    {
      ...
      /* If used by PHI, COND, CALL, or the statement itself has side effects, 
         it indicates active code  */
      if (code == GIMPLE_PHI || code == GIMPLE_COND || code == GIMPLE_CALL 
          || gimple_has_side_effects (use_stmt))
        return false;

      /* If it is a normal assignment statement, recursively check its LHS  */
      if (is_gimple_assign (use_stmt))
        {
          tree lhs = gimple_assign_lhs (use_stmt);
          if (TREE_CODE (lhs) == SSA_NAME)
            {
              if (!is_dead_computation_chain_p (lhs, visited))
                return false;
            }
          else
            {
              /* If LHS is MEM (i.e., memory Store), 
                 it has side effects and is not dead code  */
              return false; 
            }
        }
      else
        {
          /* For other unknown statement types, 
             conservatively assume it is not dead code  */
          return false;
        }
    }
...
/* Determine the order of two stmts in a basicblock   */
static inline bool
stmt_executed_before_p (gimple *stmt1, gimple *stmt2)
{
  gcc_assert (gimple_bb (stmt1) == gimple_bb (stmt2));
  if (stmt1 == stmt2) return false;

  /* Forward traverse BB to ensure the order  */
  gimple_stmt_iterator gsi;
  for (gsi = gsi_start_bb (gimple_bb (stmt1)); !gsi_end_p (gsi); gsi_next (&gsi))
    {
      gimple *cur = gsi_stmt (gsi);
      if (cur == stmt1) return true;
      if (cur == stmt2) return false;
    }
  return false; 
}

/* Replace loop_index with loop_counter to 
   reduce the cycle from 'add' + 'cmp'  to 'subs'  */
static bool
replace_loop_index_with_loop_counter(struct ivopts_data *data, class loop *loop)
{
  /* ========================================================== */
  /* 1. Find core variables : loop_index
  /* ========================================================== */
  struct iv_cand *orig_biv = NULL;
  for (unsigned i = 0; i < data->vcands.length (); i++)
    {
      struct iv_cand *c = data->vcands[i];
      if (c->pos == IP_ORIGINAL 
          && integer_onep (c->iv->step) 
          && INTEGRAL_TYPE_P (TREE_TYPE (c->iv->base)))
        {
          orig_biv = c;
          break;
        }
    }
  if (!orig_biv) return false;

  tree loop_index = orig_biv->var_before;
  /* Explicitly extract the initial value from the preheader edge  */
  tree loop_index_ori = NULL_TREE;
  gimple *def_stmt = SSA_NAME_DEF_STMT (loop_index);
  if (def_stmt && gimple_code (def_stmt) == GIMPLE_PHI)
    {
      edge preheader_e = loop_preheader_edge (loop);
      loop_index_ori = PHI_ARG_DEF_FROM_EDGE (def_stmt, preheader_e);
    }
    
  /* If a valid initial value cannot be found, abort the optimization */
  if (!loop_index_ori) 
    {
      if (dump_file && (dump_flags & TDF_DETAILS))
        fprintf (dump_file, "  [CUSTOM-COUNTDOWN] Rejected: cannot extract loop_index_ori from PHI\n");
      return false;
    }

  /* Ensure loop_index_ori is a valid register value or constant  */
  if (TREE_CODE (loop_index_ori) != SSA_NAME && TREE_CODE (loop_index_ori) != INTEGER_CST)
    {
      if (dump_file && (dump_flags & TDF_DETAILS))
        fprintf (dump_file, "  [CUSTOM-COUNTDOWN] Rejected: loop_index_ori is not SSA_NAME or INTEGER_CST\n");
      return false;
    } 

  /* ========================================================== */
  /* 2. Find Increment Statement */
  /* ========================================================== */
  tree loop_index_next = NULL_TREE;
  gimple *latch_add_stmt = NULL;
  
  {
    imm_use_iterator it1;
    use_operand_p up1;
    FOR_EACH_IMM_USE_FAST (up1, it1, loop_index)
      {
        /* Find the +1 statement and GIMPLE_COND in Latch  */
        gimple *stmt = USE_STMT (up1);
        basic_block bb = gimple_bb (stmt);
        if (!bb) continue;

        /* Do not require bb == loop->latch, 
           but require bb to be inside the loop and dominate loop->latch  */
        if (flow_bb_inside_loop_p (loop, bb) 
            ...
            && gimple_assign_rhs_code (stmt) == PLUS_EXPR)
          {
            ...
            if ((TREE_CODE (rhs1) == INTEGER_CST && integer_onep (rhs1)) ||
                (TREE_CODE (rhs2) == INTEGER_CST && integer_onep (rhs2)))
              {
                loop_index_next = gimple_assign_lhs (stmt);
                latch_add_stmt = stmt;
                break;
              }
          }
  ...
  if (!loop_index_next || !latch_add_stmt) return false;

  /* ========================================================== */
  /* 3. Find GIMPLE_COND, loop_upbound  */
  /* ========================================================== */
  tree loop_upbound = NULL_TREE;
  gimple *latch_cond_stmt = NULL;
  
  {
    imm_use_iterator it2;
    use_operand_p up2;
    FOR_EACH_IMM_USE_FAST (up2, it2, loop_index_next)
      {
        gimple *stmt = USE_STMT (up2);
        if (gimple_code (stmt) == GIMPLE_COND && flow_bb_inside_loop_p (loop, gimple_bb (stmt)))
          {
            latch_cond_stmt = stmt;
            tree op0 = gimple_cond_lhs (stmt);
            tree op1 = gimple_cond_rhs (stmt);
            
            if (TREE_CODE (op0) != SSA_NAME || !flow_bb_inside_loop_p (loop, gimple_bb (SSA_NAME_DEF_STMT (op0))))
              loop_upbound = op0;
            else if (TREE_CODE (op1) != SSA_NAME || !flow_bb_inside_loop_p (loop, gimple_bb (SSA_NAME_DEF_STMT (op1))))
              loop_upbound = op1;
            break;
          }
      }
  }
  if (!loop_upbound || !latch_cond_stmt) return false;


  /* ========================================================== */
  /* 4. Collect loop_index uses  */
  /* ========================================================== */
  auto_vec<use_operand_p> all_target_uses;
  gimple *master_body_stmt = NULL;
  
  {
    imm_use_iterator it4;
    use_operand_p up4;
    FOR_EACH_IMM_USE_FAST (up4, it4, loop_index)
      {
        gimple *stmt = USE_STMT (up4);
        basic_block bb = gimple_bb (stmt);
        if (!bb) continue; 
        ...
        if (is_gimple_assign (stmt))
          {
            ...
            if (TREE_CODE (lhs) == SSA_NAME)
              {
                auto_vec<tree> visited;
                if (is_dead_computation_chain_p (lhs, &visited))
                  {
                    if (dump_file && (dump_flags & TDF_DETAILS))
                      fprintf (dump_file, 
                               "[DEBUG] Skipping dead computation chain use (e.g., a/b/c/d) in BB %d\n",
                               bb->index);
                    continue; 
                  }
              }
        ...
        /* Skip latch_add_stmt  */
        if (stmt == latch_add_stmt) continue;

        /* If looop_idnex has uses in loop_header, give up  */
        if (bb == loop->header && gimple_code (stmt) != GIMPLE_PHI)
        {
	  if (dump_file && (dump_flags & TDF_DETAILS))
	  {
	    fprintf (dump_file, "bb == loop->header && gimple_code (stmt) != GIMPLE_PHI");
	    print_gimple_stmt (dump_file, stmt, 0, TDF_SLIM);
	  }
	  return false;
	}

        all_target_uses.safe_push (up4);
	basic_block target_loop_body = gimple_bb (stmt);
        if (!master_body_stmt && target_loop_body->prev_bb == loop->header) 
          master_body_stmt = stmt;
      }
  }
  ...
  {    
    if (dump_file && (dump_flags & TDF_DETAILS))
      fprintf (dump_file, "!master_body_stmt");
    return false;
  }
  basic_block master_loop_body = gimple_bb (master_body_stmt);
  
  /* Find the edge from loop->header to master_loop_body  */
  edge loop_body_edge = NULL;
  edge e;
  edge_iterator ei;
  
  FOR_EACH_EDGE (e, ei, loop->header->succs)
    {
      if (e->dest == master_loop_body)
        {
          loop_body_edge = e;
          break;
        }
  ...
  /* If master_loop_body is not a direct successor of header, give up  */
  if (!loop_body_edge)
    {
      if (dump_file && (dump_flags & TDF_DETAILS))
        fprintf (dump_file, "  [CUSTOM-COUNTDOWN] Rejected: master_loop_body is not a direct successor of header.\n");
      return false;
    }

  /* If header has no conditional branch, give up  */
  if (EDGE_COUNT (loop->header->succs) < 2)
    {
      if (dump_file && (dump_flags & TDF_DETAILS))
        fprintf (dump_file, "  [CUSTOM-COUNTDOWN] Rejected: header has no conditional branch (out-degree < 2).\n");
      return false;
    }

  /* If branch probability to master_loop_body >= 25%, give up  */
  if (loop_body_edge->probability.to_reg_br_prob_base () >= (REG_BR_PROB_BASE / 4))
    {
      if (dump_file && (dump_flags & TDF_DETAILS))
        fprintf (dump_file, "  [CUSTOM-COUNTDOWN] Rejected: branch probability to master_loop_body >= 25%%.\n");
      return false;
    }
  
  /* Count aster_loop_body_uses  */
  int master_loop_body_uses = 0;
  for (unsigned i = 0; i < all_target_uses.length (); i++)
    {
      if (gimple_bb (USE_STMT (all_target_uses[i])) == master_loop_body)
      {
        gimple *current_stmt = USE_STMT (all_target_uses[i]);
        if (!stmt_executed_before_p(master_body_stmt, current_stmt))
          master_body_stmt = current_stmt;
        master_loop_body_uses++;
    }
    }
  /* If master_loop_body_uses >3 , give up  */
  if (master_loop_body_uses > 3) 
  {
	if (dump_file && (dump_flags & TDF_DETAILS))
	  fprintf (dump_file, "master_loop_body_uses > 3");
	return false;
  }

  /* If inner_use is not dominated by master_bb_use, give up  */
  int inner_uses_count = 0;
  for (unsigned i = 0; i < all_target_uses.length (); i++)
    {
      basic_block u_bb = gimple_bb (USE_STMT (all_target_uses[i]));
      if (u_bb == master_loop_body) continue;
      inner_uses_count++;
      if (!dominated_by_p (CDI_DOMINATORS, u_bb, master_loop_body))
      {
	if (dump_file && (dump_flags & TDF_DETAILS))
	  fprintf (dump_file, "!dominated_by_p (CDI_DOMINATORS, u_bb, master_loop_body)");
	return false;
      } 
    }

  /* If inner_uses_count >3 , give up  */  
  if (inner_uses_count > 3)
  {
    if (dump_file && (dump_flags & TDF_DETAILS))
      fprintf (dump_file, "inner_uses_count > 3");
    return false;
  } 

  /* ========================================================== */
  /* 5. Perform SSA reconstruction (Preheader & Header)  */
  /* ========================================================== */
  
  /* 【Stick to the original signed type  */
  tree orig_type = TREE_TYPE (loop_index);
  tree type = orig_type;  
  
  edge preheader_e = loop_preheader_edge (loop);
  gimple_stmt_iterator pre_gsi = gsi_last_bb (preheader_e->src);
  
  tree one = build_int_cst_type (type, 1);
  tree zero = build_zero_cst (type);

  /*  Determine if +1 is needed based on the comparison operator */
  tree_code cond_code = gimple_cond_code (latch_cond_stmt);
  tree upbound_plus_1; 
  if (cond_code == GE_EXPR || cond_code == EQ_EXPR)
    {
      upbound_plus_1 = loop_upbound;
    } 
  else 
    {
      upbound_plus_1 = make_ssa_name (type);
      gsi_insert_before (&pre_gsi, gimple_build_assign (upbound_plus_1, PLUS_EXPR, loop_upbound, one), GSI_NEW_STMT);
      gsi_next (&pre_gsi);
    }
  

  /* Build loop_counter_ori and insert it in preheader */
  tree loop_counter_ori = make_ssa_name (type);
  gsi_insert_before (&pre_gsi, 
                     gimple_build_assign (loop_counter_ori, MINUS_EXPR, upbound_plus_1, loop_index_ori), GSI_NEW_STMT);

  /* Build loop_counter in loop_header  */
  tree loop_counter = make_ssa_name (type);
  tree loop_counter_next = make_ssa_name (type);
  
  gphi *new_phi = create_phi_node (loop_counter, loop->header);
  add_phi_arg (new_phi, loop_counter_ori, preheader_e, UNKNOWN_LOCATION);
  add_phi_arg (new_phi, loop_counter_next, loop_latch_edge (loop), UNKNOWN_LOCATION);

  /* Build current_loop_index and insert it before first use  */
  gimple_stmt_iterator body_gsi = gsi_for_stmt (master_body_stmt);
  tree current_loop_index = make_ssa_name (type);
  gsi_insert_before (&body_gsi, 
                     gimple_build_assign (current_loop_index, MINUS_EXPR, upbound_plus_1, loop_counter), GSI_SAME_STMT);

  /* Replace all the target_uses  */
  for (unsigned i = 0; i < all_target_uses.length (); i++)
    {
      use_operand_p u = all_target_uses[i];
      gimple *stmt = USE_STMT (u);
      SET_USE (u, current_loop_index);
      update_stmt (stmt);
    }

  /* Replace the original loop_latch_cond  */
  if (latch_cond_stmt)
    {
      gcond *cond = as_a<gcond *>(latch_cond_stmt);
      gimple_cond_set_lhs (cond, loop_counter_next);
      gimple_cond_set_rhs (cond, zero);
      gimple_cond_set_code (cond, LE_EXPR);
      update_stmt (cond);
    }

  /* Build loop_counter_next = loop_counter - 1 and 
     replace it with loop_index_next  */
  gimple_stmt_iterator latch_gsi = gsi_for_stmt (latch_add_stmt);
  gimple *new_latch_stmt = gimple_build_assign (loop_counter_next, MINUS_EXPR, loop_counter, one);
  gsi_insert_before (&latch_gsi, new_latch_stmt, GSI_SAME_STMT);

  if (dump_file && (dump_flags & TDF_DETAILS))
    fprintf (dump_file, "  [CUSTOM-COUNTDOWN] Successfully replaced loop_index in loop %d\n", loop->num);
			   ...
			   bitmap toremove)
{
  ...
  /* Replace loop_index with loop_counter  */
  if (flag_replace_loop_index && replace_loop_index_with_loop_counter(data, loop))
    changed = true;
