...
/* DEF_BB is a basic block containing a DEF that needs rewriting into
   ...
   Compute the subset of loop exit destinations that exit the loop containing
   DEF_BB or one of its loop fathers, in which DEF is live.  This set is
   returned in the bitmap LIVE_EXITS.
...
/* Add exit phis for the names marked in NAMES_TO_RENAME.
   Sets of blocks where the ssa names are used are stored in USE_BLOCKS.  */
...
add_exit_phis (bitmap names_to_rename, bitmap *use_blocks)
{
  unsigned i;
  bitmap_iterator bi;
  bitmap def_loop_exits = BITMAP_ALLOC (&loop_renamer_obstack);

  EXECUTE_IF_SET_IN_BITMAP (names_to_rename, 0, i, bi)
    {
      tree name = ssa_name (i);
      class loop *def_loop = gimple_bb (SSA_NAME_DEF_STMT (name))->loop_father;
	...
	for (auto exit = loop->exits->next; exit->e; exit = exit->next)
      ...
      add_exit_phis_var (name, use_blocks[i], def_loop_exits);
    }
