  ...
  for (; TREE_CODE (ref) == COMPONENT_REF; ref = TREE_OPERAND (ref, 0))
    {
      ...
      byte_offset = int_byte_position(field);
      *delta += byte_offset;
    }
...
loop_prefetch_arrays (class loop *loop)
{
  ...
  if (dump_file && (dump_flags & TDF_DETAILS))
    fprintf (dump_file, "Time %d, Ahead %d, unroll factor %d, trip count "
        HOST_WIDE_INT_PRINT_DEC "\n"
        "insn count %d, mem ref count %d, prefetch count %d\n",
        time, ahead, unroll_factor, est_niter,
...
should_analyze_func_p (void)
{
      ...
      || func_count < PREFETCH_FUNC_COUNTS_THRESHOLD)
  {
    if (dump_file && (dump_flags & TDF_DETAILS))
    {
      fprintf (dump_file, "function uid %ld total counts is %lu: "
          "rank %d > topn %d, counts %lu < threshold %u "
  ...
  {
    fprintf (dump_file, "function uid %ld total counts is %lu: "
        "rank %d in topn %d, counts %lu > threshold %u "
        "continue prefetch analysis\n",
        decl_uid, func_count,
        func_rank_info.rank, PREFETCH_FUNC_TOPN,
        func_count, PREFETCH_FUNC_COUNTS_THRESHOLD);
