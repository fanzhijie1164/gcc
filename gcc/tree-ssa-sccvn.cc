...
vn_reference_eq (const_vn_reference_t const vr1, const_vn_reference_t const vr2)
{
  ...
  if (!vr1->type || !vr2->type)
    ...
    return false;
  else if (!flag_enhance_fre
	   && VECTOR_BOOLEAN_TYPE_P (vr1->type)
  ...
  /* 5) For aggregate copies translate the reference through them if
	     ...
	     && vn_reference_op_eq (&vr->operands[i], &lhs_ops[j]))
	{
	 ...
	 will simply fail if we messed up types too badly.  */
	  ...
	  && ((flag_enhance_fre && known_ne (lhs_ops[0].off, -1))
	      || (!flag_enhance_fre && maybe_ne (lhs_ops[0].off, -1))))
	{
	  if (known_eq (lhs_ops[0].off, vr->operands[i].off))
	    i--, j--;
	  else if (vr->operands[i].opcode == MEM_REF
		   && ((flag_enhance_fre && known_ne (vr->operands[i].off, -1))
		       || (!flag_enhance_fre && maybe_ne (vr->operands[i].off, -1))))
	    {
...
init_vn_nary_op_from_stmt (vn_nary_op_t vno, gassign *stmt)
{
  ...
  if (flag_enhance_fre && gimple_bb (stmt))
  ...
  switch (EDGE_COUNT (vp1->block->preds))
    ...
    case 2:
	 ...
	 avoid hashing the block index.  */
      if (flag_enhance_fre && vp1->block->loop_father->header == vp1->block)
	;
      else if (flag_enhance_fre)
	break;
      else if (vp1->cclhs)
	...
	case 2:
	  {
	    ...
	    gcond *last1 = safe_dyn_cast <gcond *> (last_nondebug_stmt (idom1));
	    gcond *last2 = safe_dyn_cast <gcond *> (last_nondebug_stmt (idom2));
	    if (! last1 || ! last2)
	      return false;
	    bool inverted_p;
...
vn_phi_lookup (gimple *phi, bool backedges_varying_p)
{
  ...
  FOR_EACH_EDGE (e, ei, gimple_bb (phi)->preds)
	...
	{
	  if (flag_enhance_fre && ssa_undefined_value_p (def, false))
	    def = VN_TOP;
	  else if (!flag_enhance_fre
  ...
  if (from != to)
      ...
      if (currval == from)
	{
      ...
      curr_undefined = (TREE_CODE (currval) == SSA_NAME
			&& (flag_enhance_fre || !virtual_operand_p (currval))
	  ...
	  && is_gimple_min_invariant (to))
	{
	  ...
	  to = from;
	       ...
	       && !curr_undefined
	       && TREE_CODE (to) == SSA_NAME
	       && (flag_enhance_fre || !virtual_operand_p (to))
...
visit_nary_op (tree lhs, gassign *stmt)
{
  ...
  if (flag_enhance_fre)
    ...
    result = vn_nary_op_lookup_stmt (stmt, &vnresult);
  if (!result && vnresult)
...
visit_phi (gimple *phi, bool *inserted, bool backedges_varying_p)
{
  ...
  FOR_EACH_EDGE (e, ei, bb->preds)
	...
	/* Ignore undefined defs for sameval but record one.  */
		 ...
		 && ssa_undefined_value_p (def, false))
	  seen_undef = def;
	else if (!flag_enhance_fre && sameval == VN_TOP)
	  {
	    sameval = def;
	    sameval_e = e;
	  }
	else if (!flag_enhance_fre && expressions_equal_p (def, sameval))
	  sameval_e = NULL;
	else if (!flag_enhance_fre && virtual_operand_p (def))
	  {
	    sameval = NULL_TREE;
	    break;
	  }
		 ...
		 || !flag_enhance_fre)
	  {
	    ...
	    if (!flag_enhance_fre && !FLOAT_TYPE_P (TREE_TYPE (def))
  ...
  for (gimple_stmt_iterator gsi = gsi_start_bb (bb);
	...
	case GIMPLE_COND:
	  {
	    ...
	    if (flag_enhance_fre)
	      {
		lhs = lookup_equiv_head (gimple_cond_lhs (last), bb);
		rhs = lookup_equiv_head (gimple_cond_rhs (last), bb);
		ops[0] = lhs;
		ops[1] = rhs;
		val = fold_const_from_equiv_heads (2, gimple_cond_code (last),
		...
		if ((do_region && bitmap_bit_p (exit_bbs, false_e->dest->index))
		  ...
		  false_e = NULL;
		if (flag_enhance_fre && (true_e || false_e))
		  {
	...
	FOR_EACH_SSA_TREE_OPERAND (op, gsi_stmt (gsi), i, SSA_OP_DEF)
	  if (! avail.eliminate_avail (bb, op)
	      && (flag_enhance_fre || ! has_zero_uses (op)))
  ...
  if (iterate)
      ...
      while (!bitmap_empty_p (worklist))
	{
	  ...
	  if (flag_enhance_fre)
	    ...
	    idx = bitmap_clear_first_set_bit (worklist);
	  basic_block bb = BASIC_BLOCK_FOR_FN (fn, rpo[idx]);
...
pass_fre::execute (function *fun)
{
  ...
  if (!flag_enhance_fre && iterate_p)
    loop_optimizer_init (AVOID_CFG_MODIFICATIONS);
  else if (flag_enhance_fre && iterate_p)
    {
      /* Insert empty blocks as branch targets if the original target has
	 multiple predessesor, so that all predicates can be recorded. */
      basic_block bb;
      edge e;
      edge_iterator ei;
