...
/* Return true if COND_EXPR and all computations in COND_EXPR_STMT_LIST are
   invariant in the immediate outer loop of LOOP.  */

static bool
versioning_conditions_outer_invariant_p (class loop *loop, tree cond_expr,
                                         gimple_seq cond_expr_stmt_list)
{
  ...
  if (cond_expr && !expr_invariant_in_loop_p (outer_loop, cond_expr))
    return false;
  ...
  for (gimple_stmt_iterator gsi = gsi_start (cond_expr_stmt_list);
       !gsi_end_p (gsi); gsi_next (&gsi))
    {
      ssa_op_iter iter;
      use_operand_p use_p;
      FOR_EACH_SSA_USE_OPERAND (use_p, gsi_stmt (gsi), iter, SSA_OP_USE)
       if (!expr_invariant_in_loop_p (outer_loop, USE_FROM_PTR (use_p)))
         return false;
    }
		      ...
		      gimple *loop_vectorized_call)
{
  ...
  if (version_alias)
    {
      bool allow_outer_loop_versioning
       = versioning_conditions_outer_invariant_p (loop, cond_expr,
                                                  cond_expr_stmt_list);
		      ...
		      gimple *loop_vectorized_call)
{
  ...
  if (version_alias)
    {
      vect_create_cond_for_unequal_addrs (loop_vinfo, &cond_expr);
      vect_create_cond_for_lower_bounds (loop_vinfo, &cond_expr);
      vect_create_cond_for_alias_checks (loop_vinfo, &cond_expr, false);
    }
  ...
  if ((!loop_to_version->inner || loop == loop_to_version)
      && loop_vectorized_call)
    {
      gcc_assert (scalar_loop);
      condition_bb = gimple_bb (loop_vectorized_call);
      gimple_stmt_iterator gsi = gsi_last_bb (condition_bb);
      gimple *stmt = gsi_stmt (gsi);
