  ...
  FOR_EACH_EDGE (e, ei, EXIT_BLOCK_PTR_FOR_FN (cfun)->preds)
    {
      ...
      if (gsi_end_p (gsi))
	continue;
      greturn *ret = safe_dyn_cast <greturn *> (gsi_stmt (gsi));
      if (!ret)
	continue;
  ...
  if (slp_node && SLP_TREE_LOAD_PERMUTATION (slp_node).exists ())
    {
      ...
      inside_cost += record_stmt_cost (cost_vec, n_perms, vec_perm,
				       slp_node, SLP_TREE_VECTYPE (slp_node),
				       0, vect_body);
			     ...
			     gimple_stmt_iterator *gsi)
{
  ...
  if (gimple_has_mem_ops (vec_stmt) && need_ssa_update_p (cfun))
		     ...
		     gather_scatter_info *gs_info)
{
  ...
  *poffset = 0;
  if (STMT_VINFO_GATHER_SCATTER_P (stmt_info))
    {
      *memory_access_type = VMAT_GATHER_SCATTER;
      if (slp_node && SLP_TREE_CHILDREN (slp_node).length () == 0)
	{
	  if (dump_enabled_p ())
	    dump_printf_loc (MSG_MISSED_OPTIMIZATION, vect_location,
			     "unsupported SLP gather/scatter without offset "
			     "operands.\n");
	  return false;
	}
      if (!vect_check_gather_scatter (stmt_info, loop_vinfo, gs_info))
		    ...
		    tree vectype_in, stmt_vector_for_cost *cost_vec)
{
  ...
  if (!can_vec_perm_const_p (TYPE_MODE (char_vectype),
			     TYPE_MODE (char_vectype), indices))
    return false;
...
static gimple *
vect_gen_widened_results_half (vec_info *vinfo, code_helper code,
			       ...
			       stmt_vec_info stmt_info)
{
  ...
  if (op_type != binary_op)
    vec_oprnd1 = NULL;
  new_stmt = vect_gimple_build (vec_dest, code, vec_oprnd0, vec_oprnd1);
  new_temp = make_ssa_name (vec_dest, new_stmt);
					...
					int op_type)
{
  ...
  FOR_EACH_VEC_ELT (*vec_oprnds0, i, vop0)
      ...
      else
	{
      ...
      gimple_set_lhs (new_stmt3, new_tmp3);
			 ...
			 stmt_vector_for_cost *cost_vec)
{
  ...
  if (!widen_arith
      && !CONVERT_EXPR_CODE_P (code.safe_as_tree_code ())
      && code != FIX_TRUNC_EXPR
      && code != FLOAT_EXPR)
    return false;
  ...
  switch (modifier)
	...
	return false;
	  ...
	  && supportable_convert_operation ((tree_code) code, vectype_out,
					      vectype_in, &tc1))
	{
	  code1 = tc1;
	  break;
    ...
    case NARROW:
      ...
      if (code.is_tree_code ()
	  && supportable_narrowing_operation ((tree_code) code, vectype_out,
					       vectype_in, &tc1, &multi_step_cvt,
					       &interm_types))
	{
	  code1 = tc1;
	  break;
	  ...
	  for (i = 0; i < vec_num; i++)
	      ...
	      if (memory_access_type == VMAT_GATHER_SCATTER)
		{
		  if (gs_info.ifn == IFN_LAST && !gs_info.decl)
		    {
		      /* Emulated scatter, as in GCC 15 PR117557.  */
			...
			= const_offset_nunits / const_nunits;
		      unsigned stmt_index = vec_num * j + i;
		      vec_offset = vec_offsets[stmt_index / factor];
		      unsigned elt_offset
			= (stmt_index % factor) * const_nunits;
		      ...
		      for (unsigned k = 0; k < const_nunits; ++k)
			{
			  ...
			  tree ptr
			    = gimple_build (&stmts, PLUS_EXPR,
					    TREE_TYPE (dataref_ptr), dataref_ptr, idx);
...
supportable_widening_operation (vec_info *vinfo,
				code_helper code, stmt_vec_info stmt_info,
				tree vectype_out, tree vectype_in,
                                ...
                                vec<tree> *interm_types)
{
  ...
  *multi_step_cvt = 0;
  ...
  if (widening_fn_p (code))
    {
      /* Internal widening functions use either a low/high or an even/odd
	 pair of target optabs.  */
