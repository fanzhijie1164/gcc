	  ...
	  && DR_GROUP_SLP_TRANSPOSE (DR_GROUP_FIRST_ELEMENT (stmt_info)))
	{
	  ...
	  for (i = 0; i < array_num; i++)
		...
		(vinfo, first_stmt_b, aggr_type,
		 ...
		 &ptr_incr, simd_lane_access_p, bump);
	      dr_vec_info *cur_first_dr_info = get_dr_info (first_stmt_b);
	      for (unsigned int t = 0; t < ncontinues; t++)
		{
		  tree trans_vec_oprnd = result_chain[k++];
		  if (t > 0)
		    trans_dataref_ptr = bump_vector_ptr
		      (vinfo, trans_dataref_ptr, ptr_incr, gsi, first_stmt_b,
		       bump);
		  add_new_stmt_vect_store (vinfo, vectype, trans_dataref_ptr,
				       dataref_offset, trans_ref_type,
			  ...
			  gimple_stmt_iterator *gsi)
{
  ...
  for (unsigned int i = 0; i < ncopies; i++)
      ...
      for (unsigned int t = 0; t < nloads; t++)
	{
	  ...
	  tree bump = vect_get_data_ptr_increment (vinfo, gsi, cur_first_dr_info,
						   vectype_info->ltype,
						   memory_access_type);
	  bool simd_lane_access_p
  ...
  */
  ...
  for (j = 0; j < ncopies; j++)
	  ...
	  && DR_GROUP_SLP_TRANSPOSE (DR_GROUP_FIRST_ELEMENT (stmt_info)))
	{
	  ...
	  tree lvectype = vectype;
	  tree ltype = calculate_new_type (vectype, const_nunits, group_size_b,
					 nloads, ncontinues, lvectype);
	  bool this_load_permuted = false;
	  auto_vec<unsigned> group_idx;
	  generate_load_permutation (slp_node, new_group_size, group_size,
				     group_size_b, this_load_permuted, group_idx,
				     new_load_permutation);
	  slp_perm = is_slp_perm (slp_perm, this_load_permuted, nunits,
				  group_size, first_stmt_info);

	  unsigned int trans_ncopies = new_group_size / const_nunits;
	  offset_info trans_offset = { offset, NULL_TREE, dataref_offset };
	  vectype_info trans_vectype = { vectype, ltype, lvectype, ref_type };
	  if (slp_perm)
	    dr_chain.create (trans_ncopies);
	  if (nloads > 1 && ncontinues == 1)
	    new_vect_stmt_for_nloads (vinfo, trans_ncopies, nloads, group_idx,
					stmt_info, &trans_offset, &trans_vectype,
					memory_access_type, slp_perm, dr_chain,
					slp_node, gsi);
	  else
	    new_vect_stmt_for_ncontinues (vinfo, ncontinues, group_idx, stmt_info,
					   &trans_offset, &trans_vectype,
					   memory_access_type, slp_perm, dr_chain,
					   slp_node, gsi);
	  ...
	  if (slp_perm)
		...
		= SLP_TREE_SCALAR_STMTS (slp_node)[0];
	      unsigned int old_size = DR_GROUP_SIZE (perm_stmt_info);
	      DR_GROUP_SIZE (perm_stmt_info) = new_group_size;
	      vec<unsigned> old_load_permutation
		= SLP_TREE_LOAD_PERMUTATION (slp_node);
	      SLP_TREE_LOAD_PERMUTATION (slp_node) = new_load_permutation;
	      bool ok = vect_transform_slp_perm_load
		(vinfo, slp_node, dr_chain, gsi, vf, false, &n_perms);
	      DR_GROUP_SIZE (perm_stmt_info) = old_size;
